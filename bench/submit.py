#!/usr/bin/env python3
"""Submit a bench run: fixed tasks x N replicates, pinned model, throttled.

Writes bench/runs/<run-id>/manifest.json incrementally; --resume skips
(task, replicate) pairs that already have a job id, so an interrupted run
continues instead of duplicating jobs. See bench/README.md.
"""
...
def main():
    ...
    manifest["pins"] = {"model": args.model, "arm": args.arm,
                        "autonomy": "full", "tasks_file": args.tasks}
    ...
    print(f"run {args.run_id}: {len(queue)} jobs to submit "
    ...
    while queue:
        ...
        try:
            ...
            manifest["jobs"].append({
                "task": task["id"], "replicate": replicate, "job_id": None,
                "error": str(e), "submitted_at": datetime.now(timezone.utc).isoformat(),
            })
            save_manifest(manifest_path, manifest)
            continue
        print(f"submitted {task['id']} r{replicate} -> {job_id}", flush=True)
        manifest["jobs"].append({
            "task": task["id"], "family": task.get("family"),
            "replicate": replicate, "job_id": job_id,
    ...
    print(f"done: {len(manifest['jobs'])} jobs, statuses: "
