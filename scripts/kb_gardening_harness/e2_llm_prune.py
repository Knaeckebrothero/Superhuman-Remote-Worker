...
pool_anchor = sorted(
    s for s in anchor if notes[s]["type"] in NURSERY and notes[s]["status"] == "active"
)
...
sample = (
    random.sample(pool_anchor, N_ANCHOR)
    + random.sample(pool_retired, N_RETIRED)
    + random.sample(pool_unknown, N_UNKNOWN)
)
random.shuffle(sample)
print(
    f"sample {len(sample)}: anchor-nursery {N_ANCHOR}, retired {N_RETIRED}, unknown {N_UNKNOWN}; model={MODEL}"
)
...
def card(s, with_links):
    ...
    lines = [
        f"### {s}",
        f"type: {n['type']} | status: {n['status']} | author: {fm.get('author')} | created: {fm.get('created')}",
        f"description: {desc}",
        f"excerpt: {body}",
    ]
    if with_links:
        ...
        lines.append(
            f"inbound links: {len(srcs)} total, {len(act)} from active notes, {len(dur)} from active durable notes (decision/goal/plan/ticket): {', '.join(dur[:5])}"
        )
    return "\n".join(lines)
...
def ask(prompt, cards):
    ...
    for attempt in range(3):
        p = subprocess.run(
            ["claude", "-p", text, "--output-format", "json", "--model", MODEL],
            capture_output=True,
            text=True,
            timeout=600,
        )
        try:
            ...
            return {v["slug"]: v["verdict"].upper() for v in data["verdicts"]}, out.get(
                "total_cost_usd", 0
            )
...
def run(name, prompt, with_links, filter_protected):
    ...
    regret = del_set & KEEP
    agree = del_set & RETIRED
    newly = del_set & UNKNOWN
    shown_keep = len([s for s in batch_notes if s in KEEP])
    shown_ret = len([s for s in batch_notes if s in RETIRED])
    shown_unk = len([s for s in batch_notes if s in UNKNOWN])
    print(
        f"{name:6s} answered={len(verdicts)}/{len(batch_notes)} retire={len(del_set)}  REGRET={len(regret)}/{shown_keep} anchored ({len(regret) / max(1, shown_keep):.0%})  agrees-with-RETIRED={len(agree)}/{shown_ret} ({len(agree) / max(1, shown_ret):.0%})  newly-retired-UNKNOWN={len(newly)}/{shown_unk} ({len(newly) / max(1, shown_unk):.0%})  cost=${cost:.2f}"
    )
    return verdicts
...
json.dump(
    {
        "sample": sample,
        "keep": sorted(KEEP & set(sample)),
        "retired": sorted(RETIRED & set(sample)),
        "verdicts": out,
    },
    open(pathlib.Path(__file__).parent / f"e2_e3_{MODEL}.json", "w"),
    indent=1,
)
