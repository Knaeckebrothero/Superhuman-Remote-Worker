...
def note(slug, ntype, links=(), created_days_ago=30):
    ...
    body = f"# {slug}\n\nE6 lane smoke ({ntype}).\n"
    if links:
        body += "\n## Relationships\n**REFERENCES:** " + ", ".join(f"[{l}]({l}.md)" for l in links) + "\n"
    return (
...
async def main():
    ...
    vector_db = PostgresDB(
        connection_string=build_postgres_url("VECTOR_POSTGRES", fallback_env="VECTOR_DB_URL"),
    ...
    types = {"root": "decision", "linked": "learning", "orphan1": "learning", "orphan2": "state", "young": "learning"}
    rows = {}
    for key, slug in slugs.items():
        res = await materialize_knowledge_note(
            postgres_db=postgres_db, gitea_client=gitea, project_id=PROJECT, slug=slug, content=contents[key]
        )
        assert res["status"] in ("committed", "skipped"), res
        created = datetime.now(timezone.utc) - timedelta(days=1 if key == "young" else 30)
        row_id = await store.upsert_kb_note(
            kb_id=kb, note_id=slug, path=f"knowledge/{slug}.md", title=slug, note_type=types[key],
            content=contents[key], blob_sha=blob_sha(contents[key]), embedding_version=None,
            status="active", tags=["e6"], created_at=created, modified_at=created,
        )
        rows[key] = row_id
    ...
    async def statuses():
        ...
        for key, slug in slugs.items():
            r = await vector_db.fetchrow(
                "SELECT status, invalidated_at FROM knowledge_index WHERE kb_id=$1 AND note_id=$2", kb, slug
            )
    ...
    pre = await prefilter_kb_tick(
        postgres_db=postgres_db, store=store, gitea_client=gitea, kb_id=kb,
        min_age=timedelta(days=7), limit=25,
    )
    ...
    file_txt = await gitea.get_file_content(resolved.repo, f"knowledge/{slugs['orphan1']}.md")
    print("S4 orphan1 file status line:", [l for l in file_txt.splitlines() if l.startswith("status:")])
    ...
    for key, slug in slugs.items():
        ...
        if sha:
            await vector_db.execute(
                "UPDATE knowledge_index SET blob_sha=$3 WHERE kb_id=$1 AND note_id=$2", kb, slug, sha
            )
            restamped += 1
    print("S4b restamped blob_sha from tree for", restamped, "rows (sweep simulation)")
    purge = await purge_kb_tick(
        postgres_db=postgres_db, store=store, gitea_client=gitea, kb_id=kb,
        grace=timedelta(seconds=0), limit=25,
    )
    ...
    for key in ("root", "linked", "young"):
        await materialize_knowledge_note_delete(
            postgres_db=postgres_db, gitea_client=gitea, project_id=PROJECT, slug=slugs[key], reason="e6 cleanup", store=store
        )
    print("SUMMARY", json.dumps({
        "prefilter": pre, "after_prefilter": after_pre, "purge": purge, "after_purge": after_purge, "files_present_after_purge": present,
        "expect": "orphan1/orphan2 archived then purged; root/linked/young untouched",
    }))
    await vector_db.close(); await postgres_db.close()
