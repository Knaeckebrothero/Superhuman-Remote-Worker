...
for depth in (1, 2, 3, 99):
    ...
    v = cand & KEEP; u = cand & UNKNOWN
    bt = collections.Counter(notes[s]["type"] for s in u)
    print(f"R4 unreachable from roots within {depth:2d} hops: candidates={len(cand):4d} KEEP-violations={len(v):3d} new-UNKNOWN={len(u):4d} ({len(u)/len(hist_active):.0%} of active history) {dict(bt)}")
r = reach(roots, 99)
cand = {s for s in active if notes[s]["type"] in NURSERY and s not in r} - KEEP
json.dump(sorted(cand), open(pathlib.Path(__file__).parent / "e1b_r4_selection.json", "w"))
...
while frontier:
    ...
    for s in frontier:
        for l in notes[s]["links"]:
            if l in notes and l in active and l not in seen:
                seen.add(l); nxt.add(l); depths[l]=d+1
    frontier=nxt; d+=1
print("depth histogram of reachable nursery notes:", sorted(collections.Counter(depths[s] for s in seen if s in depths and notes[s]['type'] in NURSERY).items()))
