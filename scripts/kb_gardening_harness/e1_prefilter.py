...
import sys, pathlib, re, hashlib, collections, json
import numpy as np
...
inbound_active = {s: {src for src in srcs if src in active} for s, srcs in inbound.items()}
...
print(f"classes: KEEP={len(KEEP)} RETIRED={len(RETIRED)} UNKNOWN={len(UNKNOWN)} (overlap keep&retired={len(KEEP & RETIRED)})")

def report(name, selected, detail=""):
    sel = set(selected)
    v = sel & KEEP; r = sel & RETIRED; u = sel & UNKNOWN
    by_type = collections.Counter(notes[s]["type"] for s in u)
    print(f"{name:34s} selected={len(sel):5d}  KEEP-violations={len(v):4d}  agrees-with-RETIRED={len(r):4d} ({len(r)/max(1,len(RETIRED)):.0%} of retired)  new-UNKNOWN={len(u):4d} {dict(by_type.most_common(4))} {detail}")
    return sel
...
for g in groups.values():
    if len(g) > 1:
        ...
        r0 += g[1:]
report("R0 exact-dup (keep oldest)", r0, f"groups={sum(1 for g in groups.values() if len(g)>1)}")
...
def near_dup(thr):
    ...
    for i, j in zip(ii, jj):
        a, b = slugs[i], slugs[j]
        ka = (len(inbound_active.get(a, ())), -len(notes[a]["links"]), notes[a]["fm"].get("created") is None, a)
        kb = (len(inbound_active.get(b, ())), -len(notes[b]["links"]), notes[b]["fm"].get("created") is None, b)
...
def norm_title(t):
    t = t.lower(); t = re.sub(r"[#`*_]", " ", t); t = re.sub(r"\b(iter(ation)?|phase|cycle|job|v)\s*-?\s*\d+\b", " ", t)
    t = re.sub(r"\d{4}-\d{2}-\d{2}(t\d+)?", " ", t); t = re.sub(r"\d+", " ", t); return re.sub(r"\s+", " ", t).strip()
...
for g in tg.values():
    if len(g) > 1:
        g.sort(key=lambda s: (-len(inbound_active.get(s, ())), str(notes[s]["fm"].get("created") or "0")), reverse=False)
        # keep the best-linked (first after sort by -inbound asc? fix: sort desc by inbound)
        g.sort(key=lambda s: (len(inbound_active.get(s, ())), str(notes[s]["fm"].get("created") or "0")), reverse=True)
        r2 += g[1:]
report("R2 same-title moving-target twins", r2, f"groups={sum(1 for g in tg.values() if len(g)>1)}")

# R3 orphan nursery: nursery type, zero inbound links from ACTIVE notes, not protected type
r3 = [s for s, n in notes.items() if n["type"] in NURSERY and not inbound_active.get(s) and n["type"] not in PROTECTED_TYPES]
...
print(f"AUTO after G5 protection (drop anchor/live): {len(auto_strict)} of which already retired {len(auto_strict & RETIRED)}, newly retired {len(auto_strict & UNKNOWN)} = {len(auto_strict & UNKNOWN)/len(UNKNOWN):.0%} of UNKNOWN")

# what share of currently-active history notes would be left?
hist_active = {s for s, n in notes.items() if n["slice"] == "history" and n["status"] == "active"}
print(f"history active notes: {len(hist_active)}; AUTO would retire {len(auto_strict & hist_active)} ({len(auto_strict & hist_active)/len(hist_active):.0%}); R3 would retire {len(set(r3) & hist_active)} ({len(set(r3) & hist_active)/len(hist_active):.0%})")
json.dump({"auto": sorted(auto_strict), "r3": sorted(set(r3) - KEEP)}, open(pathlib.Path(__file__).parent / "e1_selection.json", "w"))
