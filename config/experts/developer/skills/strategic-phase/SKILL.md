...
The hard, token-heavy part of implementation is understanding the system and deciding what's correct — not typing. Parallelize THAT. `delegate_agent` spins up throwaway subagents that run inline with their own fresh context and return a result string; they have the full toolset (read, shell, websearch/browser, and — for spikes — writes). Good uses:
...
How to fan out: call `delegate_agent` several times in a SINGLE turn — one focused, self-contained task each (the subagent can't see your conversation, so spell out the question, the paths/URLs, and exactly what to return). N calls in one turn run concurrently.
