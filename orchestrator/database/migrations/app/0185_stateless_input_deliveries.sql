-- migration:     0185_stateless_input_deliveries.sql
-- description:   Admit durable event input on stateless session turns with
--                exact queue-lease fencing.
-- depends-on:    0184_managed_repository_legacy_reconciliation.sql
-- expected:      < 1s. Adds nullable provenance columns, backfills the small
--                input-delivery ledger from its owning thread, and installs
--                three trigger fences. No transcript or queue-row rewrite.
...
UPDATE public.thread_input_deliveries AS delivery
   SET execution_lane = thread.execution_lane
  FROM public.threads AS thread
 WHERE thread.id = delivery.thread_id
   AND delivery.execution_lane IS DISTINCT FROM thread.execution_lane;
