...
async def provision_or_assign(
    ...
    try:
        async with postgres_db.thread_advisory_lock(tid):
            ...
            if pod_ip is None:
                ...
                if not pod_name:
                    ...
                    return
                # Pod created — wait for the agent to /register itself
                # and have the orchestrator bind it.
        ...
        # Lock released. The agent is bound; now block on the actual
        # session-ready signal so the cockpit's startup card flips from
        # "booting" to "ready" exactly when the agent's _attach_session
        # finishes (the agent's /ready is gated on _session_ready()'s
        # 3-way check). Mirrors the readiness-probe tail of _do_prepare.
