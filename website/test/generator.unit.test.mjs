...
test('production with a bundled service omits its external fields', () => {
...
test('fillSecrets substitutes provided values for placeholders', () => {
  ...
  // Anchored: VECTOR_POSTGRES_PASSWORD also ends in "POSTGRES_PASSWORD".
  assert.match(secretSkeleton, /^  POSTGRES_PASSWORD: s3cret$/m);
  assert.doesNotMatch(secretSkeleton, /^  POSTGRES_PASSWORD: CHANGE_ME$/m);
});
...
test('validate requires connection details for an external service', () => {
  const hostErrs = validate('production', { ...prodBase, postgres: 'external', postgresHost: '' });
  assert.ok(hostErrs.some((e) => /postgres.*host/i.test(e)));
  const urlErrs = validate('production', { ...prodBase, mongodb: 'external', mongodbUrl: '' });
  assert.ok(urlErrs.some((e) => /mongodb.*url/i.test(e)));
});
