...
from orchestrator.services import thread_transport as thread_transport_service  # noqa: E402
from orchestrator.routers import thread_projection as thread_projection_routes  # noqa: E402
...
async def lifespan(app: FastAPI):
    ...
    attention_sleep_task = asyncio.create_task(
        run_when_leader(
            functools.partial(
                attention_sleep_service.attention_sleep_sweeper,
                db=postgres_db,
...
def _thread_admission_dependencies() -> (
    ...
    return thread_admission_service.ThreadAdmissionDependencies(
        ...
        provision_or_assign=_provision_or_assign,
        redact_thread_metadata=thread_projection_routes._redact_thread_metadata,
    )
...
app.state.thread_transport_dependencies_factory = _thread_transport_dependencies


async def _thread_input_stateless(thread: dict, content: str) -> dict[str, Any]:
    """B12 bridge: pre-extraction signature for operator_cli/
    stateless_wake_acceptance.py. Removal owner: B12.
    """
    return await thread_transport_service._thread_input_stateless(
        thread,
        content,
        db=postgres_db,
        schedule_stateless_workspace_ensure=(_schedule_stateless_workspace_ensure),
...
app.include_router(thread_projection_routes.router)


def _thread_projection_dependencies() -> (
    thread_projection_routes.ThreadProjectionDependencies
):
    """Resolve collaborators only from the application handling this request."""
    return thread_projection_routes.ThreadProjectionDependencies(
        store=postgres_db,
        vector_db=vector_db,
        resolve_cloud_session_url=_resolve_cloud_session_url,
        resolve_session_config=_resolve_session_config,
...
app.state.thread_projection_dependencies_factory = _thread_projection_dependencies
