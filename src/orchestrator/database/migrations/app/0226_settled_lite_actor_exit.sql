-- The lite tier (workspace.backend = none) is the virtual tier minus the
-- backing: one protected agent Pod, no workspace container, no binding.
-- Officers and their conferences run there. A used lite actor that exits after
-- End closes admission but before it publishes local quiescence therefore
-- needs the same settled-work recovery contract 0225 gave the virtual tier,
-- with the binding rule inverted: there must be none. Everything else is
-- unchanged — settled work only, an exact protected claim-free actor, never
-- unfinished child/input/control work — and the caller must still finish the
-- exact Pod stop/finalizer actuator first. Kubernetes absence alone is not
-- this proof.
