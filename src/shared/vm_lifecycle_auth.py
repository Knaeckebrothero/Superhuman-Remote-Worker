"""Shared authentication primitives for VM lifecycle controller messages.

The VM guest and lifecycle controller share a NATS account in existing
deployments.  Subject scoping prevents accidental cross-talk, but it does not
prove that a lifecycle command or identity response came from the controller.
This module adds a small, versioned HMAC envelope around the existing payloads.

The payload itself remains ordinary JSON for rolling-upgrade compatibility.
When ``VM_LIFECYCLE_HMAC_SECRET`` is configured, callers must reject messages
whose envelope is absent or invalid.  The operation and direction are included
in the MAC domain so a signed status response cannot be replayed as a create or
delete command.
"""
...
def configured_secret(source: Mapping[str, str] | None = None) -> bytes | None:
    """Return the configured secret, rejecting weak/non-UTF-8-empty values."""
...
def signature(
    ...
    *,
    ...
    """Return a domain-separated signature over canonical JSON."""
...
def sign_payload(
    ...
    *,
    ...
    """Copy a payload and append its authentication envelope when enabled."""
...
def verify_payload(
    ...
    *,
    ...
    """Verify an envelope; unsigned payloads are valid only in legacy mode."""
...
def unsigned_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    """Return a copy without the transport authentication envelope."""
