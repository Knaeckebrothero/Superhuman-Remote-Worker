...
def _read_stdin() -> str:
    """Read at most the byte limit plus one; a shorter stream must reach EOF."""
...
def _sniff_format(text: str) -> str:
    """Distinguish JSON bundles from YAML flow mappings and document streams.

    Only syntax is detected here; parse_documents still enforces every guard.
    """
    try:
        json.loads(text)
...
def main():
    ...
    # Reject duplicate stdin operands before reading any input.
