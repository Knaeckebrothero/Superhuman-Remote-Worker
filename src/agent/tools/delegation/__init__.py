...
is mapped to ``delegate_agent`` by ``shared.runtime.core.tool_policy.normalize_tool_policy``;
their settings keys are dropped by ``shared.runtime.core.loader.normalize_delegation_block``.
...
the ``isolation: worktree`` path of ``agent.subagents.child``.
"""
...
def create_delegation_tools(
    ...
    from agent.tools.delegation.control_plane import create_control_plane_tools
...
def get_delegation_metadata() -> Dict[str, Dict[str, Any]]:
    ...
    from agent.tools.delegation.delegate_agent import DELEGATE_AGENT_METADATA
    from agent.tools.delegation.control_plane import CONTROL_PLANE_METADATA
