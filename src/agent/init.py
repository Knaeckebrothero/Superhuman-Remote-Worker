...
Usage:
    ...
    python -m agent.init --force-reset

    # Verify workspace structure
    python -m agent.init --verify

    # Backup/restore workspace
    python -m agent.init --backup /path/to/backup
    python -m agent.init --restore /path/to/backup
"""
...
from agent.core.workspace import get_workspace_base_path  # noqa: E402
...
def setup_logging(verbose: bool = False) -> None:
    ...
    if verbose and not os.getenv("DEBUG_ALL"):
        ...
        for namespace in ("__main__", "agent", "shared"):
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m agent.init                      # Ensure workspace exists
  python -m agent.init --force-reset        # Clean all workspaces
  python -m agent.init --verify             # Verify workspace structure
  python -m agent.init --backup ./backup    # Create backup
  python -m agent.init --restore ./backup   # Restore from backup
        """,
    )
    parser.add_argument(
        "--force-reset",
