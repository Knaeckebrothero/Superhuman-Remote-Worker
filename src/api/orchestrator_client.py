...
class OrchestratorClient:
    ...
    async def create_curation_job(
        ...
        phase_data: str,
        project_id: Optional[str] = None,
        curator_config: str = "curator",
    ) -> Optional[dict[str, Any]]:
        """Create a curator subjob for knowledge extraction.

        Spawns a persistent curator that processes job artifacts (workspace.md,
        plan.md, archive/) and extracts structured knowledge notes into the
        project knowledge base via kb_write/kb_update.

        Args:
            job_id: UUID of the target job being curated
            description: Original job description
            config_name: Original agent config name (e.g., "developer")
            phase_data: Formatted phase context (retrospective content, workspace summary)
            project_id: Project UUID (required — curator needs project-scoped KB access)
            curator_config: Which expert config to use (default "curator")
        ...
        if not project_id:
            logger.error(f"Cannot create curation job for {job_id} — no project_id")
            return None

        instructions = self._format_curation_instructions(
            job_id=job_id,
            description=description,
            config_name=config_name,
            phase_data=phase_data,
            curation_mode="incremental",
            curation_phase="initial",
        )
        if not instructions:
            logger.error(f"Failed to load curation instructions template for job {job_id}")
            return None

        curation_description = (
            f"Curate knowledge from job {job_id} ({config_name}). "
            f"Extract decisions, learnings, and state into the project knowledge base."
        )

        context = {
            "curation_target": job_id,
            "original_description": description,
            "original_config": config_name,
            "curation_round": 0,
        }
        ...
        payload: dict[str, Any] = {
            ...
            "priority": 5,  # Lower priority than verification (10)
        }
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                curator_job_id = result.get("id", "unknown")
                logger.info(
                    f"Created curation job {curator_job_id} for target job {job_id}"
                )
                return result
            else:
                logger.error(
                    f"Failed to create curation job: {response.status_code} - {response.text}"
                )
                return None

        except httpx.RequestError as e:
            logger.error(f"Failed to connect to orchestrator for curation job: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error creating curation job: {e}")
            return None

    @staticmethod
    def _format_curation_instructions(
        job_id: str,
        description: str,
        config_name: str,
        phase_data: str,
        ...
        Args:
            ...
            description: Original job description
            config_name: Original job's config name
            phase_data: Formatted phase context
            curation_mode: "incremental" (per-phase) or "final" (post-approval)
            curation_phase: Phase identifier (e.g., "phase_1", "final")
        ...
        template_path = None
        search_paths = [
            Path(__file__).parent.parent.parent / "config" / "experts" / "curator" / "curation_instructions.md",
            Path(__file__).parent.parent.parent / "config" / "templates" / "curation_instructions.md",
        ]
        ...
        if not template_path:
            logger.error(
                f"Curation instructions template not found. "
                f"Searched: {[str(p) for p in search_paths]}"
            )
            return None
        ...
        try:
            ...
            logger.error(f"Failed to read curation template {template_path}: {e}")
            return None
        ...
        try:
            ...
            logger.error(f"Curation template has unknown placeholder: {e}")
            return None
