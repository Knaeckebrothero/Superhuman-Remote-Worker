...
**Fast-follows status (updated 2026-06-29):** the dashboard's own fast-follows (by-provider, Export CSV, per-job LLM, Edit-Rates UI, live RPM/TPM, sparklines) have **not** landed. **But LLM cost is now REAL** — the adjacent route-all-models work (`27d026ac`, P3, live on dev) makes `materialize_llm_usage` append a `unit='request'` / `cost_usd=spend` row to `usage_events` for paid gateway traffic, so the dashboard's cost cells + headline `total_cost_usd` show real dollars for routed paid models (dev: codex GPT-5.5 $100.79). Cost reads **"—"** only where genuinely unpriced: free homelab models (`spend=0`), `category='compute'` (vcpu/gib-hour — `usage_rates` still empty), and the local demo seed (no cost rows). The dashboard code is unchanged (it sums `cost_usd` generically — the new `(llm, request)` line flows straight through). See `docs/features/route_all_models_through_litellm_gateway.md`.
