...
## Current state — as-built (updated 2026-06-26)

**M1 is DONE — code-complete, k3d-verified, LIVE-verified on the dev cluster (`main`) under real traffic, and PUSHED to `origin/develop`** (tested with PostgresContainer via the podman socket; two-replica failover verified on local k3d AND on dev). **`replicas: 2` is correctness-safe AND graceful-failover-safe, and is now the chart default.** The push workflow ran ruff auto-format + rewrote the commit SHAs, so the SHAs in the table below are **pre-push and stale — match commits by subject in `git log`, not by hash.** The per-task bodies further below are the *original* plan; where the build diverged, the as-built note is authoritative. Verification record: `docs/tests/orchestrator_m1_leader_election_verification.md`.

**What remains: nothing for M1.** The live (dev) two-replica run PASSED 2026-06-26 (graceful + hard kill, zero serving blackout, in-flight job survived 3 kills + a rollout, PDB protects the leader) and caught + fixed two runtime bugs k3d structurally couldn't — uvicorn graceful-shutdown hang (fix: `--timeout-graceful-shutdown 10` in both orchestrator Dockerfiles) and IMAP poller 1 Hz log-spam (fix: park on `shutdown_event`); graceful failover is now ~33 s with a clean step-down (was ~66 s). The chart default was then flipped to `orchestrator.replicas: 2` + `pdb.minAvailable: 1` (M4/Phase-5 declare-done; commit subject `feat(orchestrator): default replicas:2 + PDB minAvailable:1`). **Next HA milestone if pursued: the NATS-cluster slice of M3 (coordination-plane SPOF) >> M2 active-active scale-out (deferred — throughput only, removes no SPOF).**

| Task | Status | Commit | As-built / deviations from the plan below |
|---|---|---|---|
| 1. Leader-election primitive | ✅ DONE | `bcd90dca` | `database/lock_ids.py` (`LEADER_ID = 0x5352575F4C454144` "SRW_LEAD") + `services/leader_election.py` (`run_as_leader`, module-level `is_leader` event). 3 PostgresContainer tests in `tests/test_leader_election.py`. As planned. |
| 2. Lifespan wiring + loop gating | ✅ DONE | `1c38dc6b` | **Deviation:** rather than a per-tick `is_leader` gate inside each loop, added a `run_when_leader(make_coro, shutdown, poll_seconds=1.0)` wrapper (in `leader_election.py`) applied at the lifespan **registration site** — loop bodies untouched; it cancels the loop on leadership loss within ~1s. The 9 singleton loops are wrapped; `_trigger_dispatch` gated on `is_leader`. `run_when_leader` unit-tested (no Postgres). Chosen as lower-risk than editing 9 functions in the 23k-line monolith. |
| 3. Atomic CAS job claim | ✅ DONE | `7deea369` | `PostgresDB.claim_job_for_agent` — CAS `UPDATE jobs SET status='processing', assigned_agent_id WHERE id=$1 AND assigned_agent_id IS NULL AND status IN ('created','paused') RETURNING id` (mirrors `update_job_status` uuid/`CURRENT_TIMESTAMP`). **Integrated at the dispatcher Phase-1 call site (`main.py` ~:4096, claim-before-notify, guards both dispatch + resume) — NOT inside `_dispatch_job_to_agent`** (leaves its failure handling + the manual-assign caller at `main.py:11852` untouched). `tests/test_job_claim.py` (exactly-one-wins + rejects non-dispatchable). **Ref fix:** `get_dispatchable_jobs` is at `postgres.py:2569` (plan/doc said `:2449`). **Deferred to M2:** `FOR UPDATE SKIP LOCKED` on the candidate scan — the CAS claim is the correctness guard; SKIP LOCKED is a throughput optimization, not needed for safety. |
| 4. Keepalive tuning + pooler guard | ✅ DONE | `a14abe6c` | `helm/templates/databases/postgres.yaml`: `-c tcp_keepalives_idle/interval/count=10/10/3`. Pooler warning in `helm/values.yaml` + `helm/values.example.yaml`. No `values.schema.json` change needed (schema is permissive). helm render + lint verified. As planned. |
| 5. Defense-in-depth loop hardening | ✅ DONE | `db86e13b`, `c42df854`, `1965aa78`, `ecc20a6d` | Shipped as 4 focused commits (not the plan's single commit), each with a DB-level atomic-claim test. **Deviation (imap):** used a dedicated `processed_inbound_emails` claim table (migration 0037) instead of retrofitting `UNIQUE(email_message_id)` onto the hot `message_log` — lower blast radius, no dedup-of-existing-rows, no `CONCURRENTLY`; the poller claims before routing (kept the early `message_exists_by_email_id` read as a fast path). Delegation: `claim_delegation_resume` CAS (waiting→paused) at the final transition. Digest: `claim_pending_notifications` (claim-then-dispatch, release on failure). Notify: partial unique index `uq_tn_sent_request_kind` (migration 0038, dedups pre-existing first) + `claim_sent_notification` claim-before-send. 11 tests across 4 files. |
| 6. Two-replica verification + docs | ✅ DONE — k3d run PASSED (2026-06-26) | _(by subject)_ | Verification record `docs/tests/orchestrator_m1_leader_election_verification.md` (16 unit tests PASS + the k3d two-replica run). `orchestrator_ha_scaling.md` Phase-1 checklist + matrix M1 row + the two stale "hard blocker" assertions updated to shipped. **k3d two-replica run executed and PASSED:** exactly one leader (single advisory-lock holder), `run_when_leader` gates the 7 singletons to the leader only (follower runs none), failover ×2 (lock auto-releases on graceful step-down → survivor re-acquires ~20s), warm-follower loop healthy. **Live (dev) two-replica run DONE 2026-06-26** (graceful + hard kill, zero serving blackout, in-flight job survived); it also caught + fixed 2 runtime bugs (uvicorn graceful-shutdown hang; IMAP log-spam — see the verification doc's "live dev cluster" section). Nothing remains for M1. |
| 7. (post-build) flattened-image import fix | ✅ DONE — found BY the k3d run | _(by subject: "use flattened-image import paths")_ | **The k3d run caught a deploy-blocking bug the 16 unit tests structurally could not:** the lifespan leader-election imports used the package-prefixed form (`from orchestrator.services.leader_election …` / `…database.lock_ids`), which resolves at the repo root (unit tests pass) but crashes the deployed flattened `/app` image with `ModuleNotFoundError: No module named 'orchestrator'`. Fixed to plain top-level imports (`from services.… import`, `from database.… import`) matching house convention. **Lesson: unit-test-green ≠ boots-in-cluster — any new top-level import in orchestrator code must use the flattened form, and a two-replica k3d smoke is mandatory before trusting lifespan wiring.** |
...
**M1 is COMPLETE (2026-06-26):** ~~push `develop`~~ → ~~local k3d two-replica run~~ → ~~live (dev) two-replica failover test (incl. hard-kill keepalive path)~~ → ~~flip `orchestrator.replicas: 2` (Phase 5 / M4)~~ — all done. The live run additionally caught + fixed 2 runtime bugs (uvicorn graceful-shutdown hang; IMAP log-spam); the chart default is now `replicas: 2` + `pdb.minAvailable: 1`.
...
- **Was `replicas: 1` during the M1 build** — M1 makes 2 *safe*; the count flip to 2 was M4/Phase 5 (**done 2026-06-26**, after the live dev verification).
