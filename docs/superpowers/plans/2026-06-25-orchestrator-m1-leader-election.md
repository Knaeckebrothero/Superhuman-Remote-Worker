...
**All tasks code-complete** (committed on `develop`, unpushed; tested with PostgresContainer via the podman socket). **`replicas: 2` is correctness-safe.** The only thing left is the two-replica failover *run* (Task 6 Step 1, deferred — k3d cold / live wants a quiet window) + the `develop` push. The per-task bodies further below are the *original* plan; where the build diverged, the as-built note in this table is authoritative. Verification record: `docs/tests/orchestrator_m1_leader_election_verification.md`.
...
| 6. Two-replica verification + docs | ✅ DOCS DONE; run deferred | _(this commit)_ | Verification record `docs/tests/orchestrator_m1_leader_election_verification.md` written (16 tests PASS recorded); `orchestrator_ha_scaling.md` Phase-1 checklist + matrix M1 row + the two stale "hard blocker" assertions updated to shipped. **Step 1 (two-replica failover run) deferred:** local k3d is cold (full cold-start needed) and the live run wants a quiet window — recorded in the verification doc's "Still owed" section, mirroring the M0 precedent. |
...
**To finish M1 (all that remains is operational, no code):** push `develop` → bring up local k3d (or wait for it) and run the two-replica failover test → run it on dev in a quiet window → then flip `orchestrator.replicas: 2` (Phase 5 / M4) once both pass.
