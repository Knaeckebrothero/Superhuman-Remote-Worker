...
**M1 is code-complete, k3d-verified, and PUSHED to `origin/develop`** (tested with PostgresContainer via the podman socket; two-replica failover verified on local k3d). **`replicas: 2` is correctness-safe.** The push workflow ran ruff auto-format + rewrote the commit SHAs, so the SHAs in the table below are **pre-push and stale — match commits by subject in `git log`, not by hash.** The per-task bodies further below are the *original* plan; where the build diverged, the as-built note is authoritative. Verification record: `docs/tests/orchestrator_m1_leader_election_verification.md`.

**What remains (operational, no code):** the **live (dev) two-replica run** (incl. the hard-kill keepalive path — k3d only did graceful delete) + the deliberate **`orchestrator.replicas: 2` flip** (Phase 5 / M4). The `develop` push is done; dev picks up the new image via Fleet.
...
**To finish M1 (all that remains is operational, no code):** ~~push `develop`~~ (done 2026-06-26) → ~~local k3d two-replica run~~ (done, PASSED) → run the live (dev) two-replica failover test in a quiet window (incl. the hard-kill keepalive path) → then flip `orchestrator.replicas: 2` (Phase 5 / M4).
