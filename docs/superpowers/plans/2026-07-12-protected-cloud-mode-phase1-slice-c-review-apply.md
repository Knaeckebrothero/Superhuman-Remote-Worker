...
- Tar access: download `upper.tar` bytes once per instance (`get_blob`), hold `tarfile.open(fileobj=io.BytesIO(...))` lazily; member name for `path` is `upper/{path}`. **Content binding (multi-replica torn-pair defense, added 2026-07-12):** before first tar use, verify `hashlib.sha256(tar_bytes).hexdigest() == manifest.get("tar_sha256")`; on mismatch OR absent key treat the whole staging as missing (`summary()`/`file()` return None) — a manifest must never be served against a tar it doesn't describe. Test: manifest hash ≠ tar bytes → `summary()` is None.
...
3. Build `UpperdirDiffSource`; `summary()` None → `StagedApplyError(410, {"code": "staging_missing"})` (DB says staged but S3 blobs gone — also covers the tar_sha256 content-binding mismatch, which UpperdirDiffSource surfaces as a missing summary; the user restages). Test: manifest whose `tar_sha256` doesn't match the tar → apply raises 410 and writes NOTHING to the cloud.
