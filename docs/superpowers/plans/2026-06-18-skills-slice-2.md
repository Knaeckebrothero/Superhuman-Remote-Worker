...
> **Status: ✅ Slice 2 COMPLETE & live-verified on k3d (2026-06-18).** All 8 tasks landed on `develop`. 15 new unit tests green (`test_skill_resolution.py` 7, `test_skill_runtime.py` 5, `test_skill_tool.py` 3) + persona/hydrate/real-template/lite-gating regression suites green. Both runtime halves verified **in the deployed k3d images**: orchestrator (`resolve_skill_menu` + `resolve_config(skills=…)` attach the bundled `hello-skill` menu + 318-byte SKILL.md to the blob) and agent (hydration → fenced `<available_skills>` block in the real `systemprompt.txt` → materialization incl. `references/` → `use_skill` loads the body / friendly miss). **Full autonomous run confirmed on k3d:** a real LLM job (gemma-4) was dispatched to a freshly-provisioned agent; the orchestrator attached the `hello-skill` menu+files to its frozen `resolved_config`, the agent materialized `skills/hello-skill/SKILL.md` into its workspace (SSH/RemoteBackend), and the agent **called `use_skill("hello-skill")` and received the SKILL.md body** (audit iter 14, step 32). See As-built notes for the dispatch gotcha that initially blocked it.
...
- **Full autonomous run — CONFIRMED.** Job `3171f45c` (config `default`, gemma-4 via `https://ai.h4ll.app/v1`) ran to completion on a freshly-provisioned agent (`srw-agent-j-f202b527`). Audit trail:
  - orchestrator dispatch stored `resolved_config.skills.menu = [hello-skill]` + the 318-byte SKILL.md (`_gather_in_scope_skills` + `resolve_config(skills=…)` live, not in-image);
  - agent loaded the `use_skill` tool, materialized `skills/hello-skill/SKILL.md` via SSH (`agent.py:2128`);
  - **iter 14 / step 32**: `use_skill(skill_name="hello-skill")` → `result: "[skill: hello-skill]\n\n---\nname: hello-skill\ndescription: Use when the user explicitly asks to test the skills system end-to-end.\n---\n# Hello Skill…"`; the agent then completed its todos and finished.

**The dispatch gotcha that initially blocked it (root cause — NOT agent saturation):** a job created via `X-Internal-Key` is an *internal call*, so `create_job` skips the `job.user_id = caller.id` assignment and `X-MCP-User-Id` is ignored — `user_id` must be in the **body**. With `user_id=null`, the capability-grants gate resolves to deny-by-default and the job's (and its scholar subjob's) `tools.shell`/`delegation`/`autonomy=full` are denied → the job sits in `waiting` forever and `resolved_config` is never stored. Fix for testing: put `user_id=<admin uuid>` in the body (`is_admin` short-circuits `evaluate()`), and disable the heavy scholar subjob with `config_override={"scholar": {"enabled": false}}` so the parent agent (carrying the test task) runs directly. Agent provisioning itself was fine — `active_counts_by_purpose` counts pods, not the stale offline rows.

**Follow-up (not Slice 2):** the interactive/session prompt path is wired identically (`_resolve_session_config` passes `skills=`), but a live session attach wasn't run — a quick future check.
