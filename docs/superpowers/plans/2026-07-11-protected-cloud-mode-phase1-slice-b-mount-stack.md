...
## Post-Slice-B manual validation — PASSED 2026-07-12 (k3d, real Nextcloud 31.0.14)

Live validation is a manual step on k3d against real Nextcloud (design §11.4, matching Slice A's method): tilt live-syncs the code; create a protected session and verify (1) `engage_ro_mount` passes the cured canary probe and persists a `cloud_ro_mounts` row; (2) the workspace pod mounts the RO rclone lower at `/cloud/lower` and fuse-overlayfs at `/cloud/merged`; (3) `echo x > workspace/cloud/probe.txt` lands in `/home/agent-host/.overlay/upper`, NOT in the cloud; (4) a snapshot captures the upperdir with whiteouts intact and 0 cloud bytes; (5) refresh + heal behave per §11.2.

**Result: all 5 green.** Two things the run surfaced and cured:

- **The groupfolders floor was branch-blind (commit `92f23737`).** The memory said "bump dev's groupfolders 19.1.18 → ≥ 20.1.2 first" — but that is impossible on NC 31: groupfolders ships one branch per NC major (19.x = NC 31, 20.x = NC 32), and GHSA-2vrq-fhmf-c49m was patched in **every** branch (…/18.1.8/19.1.8/20.1.2). A fully patched NC 31 (groupfolders 19.1.18) was being refused forever. The floor is now per-branch (`GROUPFOLDERS_PATCHED` in `ro_probe.py`); 19.1.18 passes.
- **Three side-channel probes over-refused (commit `8e825983`).** The RO guarantee held live (PUT/finalize-into-mount → 403, versions-restore rejected), but: uploads-finalize targeted the reader's own home (a legitimate 201) instead of inside the mount (403); tus-create only ever hit the reader's own uploads root (removed — the finalize MOVE is the real test); trash-restore's RO denial is a Nextcloud **500**, not a clean 403, so the probe now verifies the *effect* (item stays in the reader's trashbin) and the canary seeds a deterministic groupfolder-trashed item.

Evidence captured: RO lower rejects direct writes ("Read-only file system"); the staged write never appears in the NC groupfolder; the whiteout is a char device (0,0) that survives the capture tar and re-extracts into the real btrfs restore target (`/home/agent-host`; note `/tmp` is the container overlayfs and blocks nested mknod — irrelevant since restore extracts onto the emptyDir); `rclone rc vfs/refresh` → OK and the overlay `-u`/remount cycle preserves the upperdir. This gate clears Slice B for the Slice C review/apply UI.
