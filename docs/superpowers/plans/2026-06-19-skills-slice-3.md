...
**Verification (Task 7) — in the deployed orchestrator image, NOT a separate full autonomous run.** Rationale: the enforcement nudge→read loop and the workspace write-path are **unchanged shipped behavior** (Slice 2 proved skill materialization over SSH; the file-based `todo_guide` gate runs in prod today). Slice 3 changes only the gate's target *path* and the content *source*, both proven in-image under both flag states:
...
**Deferred / follow-up:** (a) **per-family skill variants** — the real fix to restore the gpt_oss todo guide; the blocker for any per-family skill. (b) DB/project **bound** skills (this slice is bundled-only). (c) materializing a bound skill's non-`SKILL.md` files via the binding channel (the two guides are single-file). (d) A full autonomous LLM run was not done (the enforcement loop is unchanged shipped behavior); worth a smoke run if a future change touches the nudge/read path.
