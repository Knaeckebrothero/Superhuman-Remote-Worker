 ...
 *   - vertical staff
 *   - eagle wings as chevrons (two ranks)
 *   - SPQR-style banner
 *   - base
 *
 * Inherits color from `currentColor` for the staff / banner / base, and uses
 * `--accent-color` for the finial dot and chevrons. Override per-theme by
 * setting `color` (CSS) on a wrapper element.
 *
 * Usage:
 *   <srw-legion-mark [size]="24"></srw-legion-mark>
 *
 * In the sidebar:
 *   <div class="sidebar-brand" style="display: flex; gap: 10px; align-items: center;">
 *     <srw-legion-mark [size]="22"></srw-legion-mark>
 *     <div>
 *       <div class="brand-name">SRW</div>
...
export class LegionMarkComponent {
  /** Square edge length in pixels. Default 24. */
  @Input() size: number = 24;

  /** Optional aria-label — set when the mark is the only brand identifier
   * present (e.g. mobile collapsed sidebar). Leave undefined when accompanied
   * by a text wordmark — the SVG is then decorative. */
