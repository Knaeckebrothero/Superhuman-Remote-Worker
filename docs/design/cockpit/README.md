...
**PWA assets are not mirrored here.** They live only where they ship: favicons and the
manifest in `cockpit/public/`, the icon set and OG image in `cockpit/src/assets/`, the head
snippet already pasted into `cockpit/src/index.html`, and the microcopy under the `pwa`
namespace of `cockpit/src/assets/i18n/{en,de-DE}.json`. The cockpit registers a service worker
via `@angular/service-worker` configured in `cockpit/ngsw-config.json`; the offline and
update-available banner is `cockpit/src/app/shell/pwa-banner/`.

A `design/asset-pack/` folder used to hold a second copy of all of it, described as "the
originals" and requiring every icon regeneration to be applied twice. Predictably it drifted —
by the time it was deleted (2026-08-17) its manifest and head snippet still carried the
pre-Travertine `#9c1f2e` brand red. Regenerate into `cockpit/` and nowhere else.
...
5. Drop a palette doc into `themes/` so future devs know what your theme is *for*, not just what colors it uses.
