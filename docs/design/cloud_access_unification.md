...
4. ⏳ **Live RO probe** — run the §3.3 fail-closed probe against real Nextcloud (≥28.0.3) and OpenCloud instances; verify every mutating verb 403s for the dedicated RO identity. **→ §11.4: OPEN — NOT RUN** (no spike plan task covered it); the probe module + 16 tests exist, but a live run needs the Phase-1 RO identity + a canary-fixture cure (synthetic ids land inconclusive → refuse) and live NC/OpenCloud validation.
...
- **`enumerate_diff` fidelity: faithful.** The Task-2 enumerator read real fuse-overlayfs-over-rclone upperdirs with zero errors, mapping char(0,0) / `.wh.` / opaque-dir markers correctly. It was hardened **twice** from live data — skip phantom `.wh..opq` char bookkeeping nodes, and fail loudly (`ValueError`) on malformed markers — both now permanent with tests.
