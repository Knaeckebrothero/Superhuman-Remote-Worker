...
**Status:** direction doc (landscape + agreed direction, **not** an implementation spec) — 2026-07-07. **Research round 2026-07-09 (§10):** 12-agent codebase+web sweep, verdict **viable with changes** — §3.1/§3.3/§3.4 corrected in place, §6 reduced to the genuinely hands-on spike set, §8.1 lists the three design decisions still needed before a spec.
...
### 8.1 Design decisions needed before the spec

1. **Staged-bytes transport.** The upperdir (the diff) is pod-local; review UI and apply are orchestrator-side. How does the orchestrator read old/new bytes + whiteouts — per-request SSH reads from the live pod, or push the upperdir (a small tar of deltas + whiteouts) to the snapshot S3 bucket at stage boundaries? This also decides whether review/apply survives pod death ("review at your leisure" vs pod lifetime). *Lean:* upperdir→S3 push at stage boundaries — decouples review from the pod, doubles as the §7 data-loss mitigation, and apply reads from S3 with orchestrator creds.
2. **Protected-OFF mechanism.** Direct rw mount (today's path survives alongside) vs overlay-with-auto-apply (one write path, but write latency + apply pipeline on every write). *Lean:* direct rw mount; unification = one mount plumbing + policy layers, and the recovery floor covers live mode.
3. **Etag-baseline scope.** Full-tree PROPFIND at mount (simple, correct; cost unknown at scale — spike item 5) vs touched-paths-only (cheap, racy). *Lean:* full-tree for project-folder scale in v1; measure before extending to user-home scale.
...
7. **RO identity blast radius + GC** — one `srw-reader` per deployment makes extracted RO creds a cross-tenant *read* key; per-user/per-mount identities and rotation need analysis. RO shares minted per mount will leak like headscale nodes did ([[srw_agent_headscale_ephemeral_leak]] class) unless revocation is designed in from day one.
