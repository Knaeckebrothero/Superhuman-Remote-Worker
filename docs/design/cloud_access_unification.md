...
2. **RO identity on prod's shared Keycloak** — creating the `srw-reader` machine user needs KC admin capability prod-private may not have; verify. **Phase-1 Slice A status (2026-07-09):** the OpenCloud RO-reader path is implemented against dev-KC (`OpenCloudBackend.ensure_ro_reader` creates a LibreGraph user linked to the configured issuer; `mint_ro_grant` invites it to the Space with the **Viewer** role, weight 40) and unit-tested with a mocked LibreGraph fake. Still OPEN before OC may engage: (a) provisioning the backing **Keycloak** machine user on prod-private's shared KC (this question), and (b) live Viewer-role RO verification via the §11.4 canary probe against a real OpenCloud instance. Nextcloud RO-reader provisioning is implemented + unit-tested and is the spike-validated backend.
