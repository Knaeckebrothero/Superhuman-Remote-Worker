...
| **Workspace** (emptyDir by default, PVC under `workspace.pvcEnabled`; SSH) | filesystem → S3 snapshot of `/home/agent-host` (happy path, reachable only); container stdout = sshd | with the pod when unreachable **and the volume is emptyDir**; stdout always lost | `browser-exec` log lives at `/tmp/browser-exec.log` and `/tmp/*` is **excluded from the snapshot** (`snapshot_service.py:291`) — the one workspace log we'd reach for (CDP failures) is both excluded and dies with the pod. *(2026-08-04: on a PVC-backed workspace `/home/agent-host` survives the pod and reattaches, so filesystem logs under it are recoverable; `/tmp` is still pod-local and still lost.)* |
