...
**The overlay exposes `.inner`** — the unwrapped backend — for the two classes of caller that must bypass virtualization: *sentinel probes* (below) and **non-tool-layer consumers**. Cloud sync is the latter: it walks and writes the user's real workspace, so `WorkspaceSyncBase` unwraps once in `__init__` and no method below it can reach a virtual path. Walking the overlay instead merged `contacts/` into the root listing and uploaded contact names, emails and phone numbers to the user's cloud folder — the exact leak this feature exists to prevent — and the return pull then raised `VirtualPathError`, which the strict coordinator turned into "cloud sync silently off for the rest of the session". Unwrapping at the seam, rather than adding names to `SYNC_IGNORE_PATTERNS`, is what stops the next provider from reintroducing both.

**The overlay must survive a backend swap.** A workspace-tier upgrade replaces the real backend mid-run; use `WorkspaceManager.swap_backend()`, which rebinds the overlay and preserves the registered providers, never `manager._backend = new_backend`.
...
**Sentinel probes — the one hazard.** `task_brief.md`'s *existence* was used as a proxy for "this workspace still has its seeded content" (`agent.py:2011`, `agent.py:2159` `if resume and _backend_has("task_brief.md")`), which feeds resume and re-seed decisions in the blast radius of the known unseeded-workspace bug. A virtual `task_brief.md` always exists, so a naive migration makes a freshly-wiped pod look seeded — **every such probe must be retargeted at `overlay.inner`** (via `unwrap_backend`). But retargeting alone is not enough: virtualizing `task_brief.md` removes its last *writer*, so a probe for it answers "unseeded" **forever**, which is the opposite failure and just as destructive — the VM/sandbox re-seed fires on every resume and `recover_to_phase` overwrites `checkpoint.db` / `plan.md` / `todos.yaml` / `archive/`, silently rewinding a same-pod resume, while the resume gate that protected content-bearing git-less workspaces becomes dead code and such a workspace falls through to `initialize()`'s `rm -rf`.

So the sentinel is a **dedicated marker file**, not an agent-facing one: `.srw_seeded` (`src/core/backends/seed.py`), written by `mark_workspace_seeded()` on the fresh-init path — exactly where the real `task_brief.md` used to be written, after every resume branch has returned — and read by `workspace_is_seeded()` through `unwrap_backend()` at both probe sites. It is purpose-named, adds no agent surface, and is unaffected by Slice 2. It is gitignored on loop `main` alongside the other framework scaffolding, matching the old sentinel's treatment.

**Degradation for in-flight workspaces:** a workspace seeded before this shipped has no marker, so `workspace_is_seeded()` also accepts a real `task_brief.md` as evidence. That deliberately degrades toward the **safe** branch — no wipe, no silent rewind — for a workspace already running; the cost is only that a genuinely wiped legacy workspace which still somehow holds a stale `task_brief.md` skips a re-seed. New workspaces get the marker on their first boot, so the legacy leg ages out. A probe error counts as "not found" for that name only (the same fail-safe the call sites used before), so one transient failure cannot on its own condemn a seeded workspace.

Enumerating and retargeting these probes is a required, tested step of Slice 1 — not a follow-up.
...
**What must be verified:** cloud-sync (`src/services/cloud_sync/base.py` lists `plan.md` in `SYNC_IGNORE_PATTERNS`) deliberately operates on the **real** filesystem — Slice 1 made `WorkspaceSyncBase` unwrap the overlay (see *Architecture*), so it will not see a virtual `plan.md` at all. Slice 2 must therefore decide separately how (or whether) a DB-backed `plan.md` reaches the user's cloud folder; "it syncs because the overlay serves it" is not available, and re-wrapping cloud sync would reopen the contacts PII leak. Todos additionally "restore from the LangGraph checkpoint (not disk)" per `job_provisioning.py:36` — Slice 2 must reconcile checkpoint restore with DB truth (DB is authoritative; checkpoint restore writes through the provider).
...
- **Boot sweep of leftovers.** `tools/`: if a real `tools/README.md` exists and its first line is the generated marker (`# Available Tools`), delete the directory — old snapshots converge and the shell stops showing stale docs; no marker → leave it (user's own dir) and log. `instructions.md` / `task_brief.md`: **left in place** — an earlier draft of this spec said "delete real copies unconditionally (system-authored; the provider is authoritative)", and implementing that would have deleted the last real `task_brief.md` on a legacy workspace, which is the fallback evidence `workspace_is_seeded()` accepts for workspaces seeded before `.srw_seeded` existed (*Sentinel probes*, below). A shadowed real copy is harmless — the provider owns the path, so the agent never reads it — while deleting it can cost a pre-marker workspace its only proof of seeding. `sweep_legacy_tools_dir` is therefore the only sweep implemented. Writable paths use mtime reconciliation instead (above). Sweeps are logged and non-fatal.
...
The **cloud-sync round-trip (the PII-leak scenario) was gated and PASSED** on local k3d 2026-08-04, driven through Cockpit with a real contact whose notes carried a canary string: providers and sync both live, zero sync errors across turns, no canary anywhere in the cloud store, and a restart whose initial pull succeeded rather than silently disabling sync. Remaining open gaps, in priority order: the two destructive resume paths the `.srw_seeded` sentinel guards (the only remaining gap whose failure mode is silent data loss), a real tier upgrade with the overlay live, and subagent-reader isolation. The kill-switch smoke is obsolete — the switch was removed 2026-08-07 and replaced by a stronger boot guarantee in `src/graph.py`.

**That gate found a bug** (`docs/issues/session_contacts_never_register_on_default_project.md`): a session whose project came from `threads.project_id` rather than a `thread_mounts` row never registered `contacts/` at all — agents saw no contacts, silently. **Resolved on `develop`** by `4f54f599` (2026-08-05), which widened `_thread_project_ids`' backfill to consult the `threads.project_id` column; the client guard was never changed, so the Registration paragraph above still holds as written. Re-verified live on local k3d 2026-08-09 (session `da1995b3`) — `contacts/` registers and `contacts/README.md` reads back the linked contact. **Not yet on `main`**, so prod carries the bug until the next release cut.
...
- **2026-07-30:** The seeded-content sentinel becomes a dedicated marker, `.srw_seeded`, rather than `task_brief.md`. Virtualizing `task_brief.md` removed its last writer, so retargeting the probes at the real backend (correct in itself) left them answering "unseeded" forever. Purpose-named, not agent-facing, unaffected by Slice 2. A real `task_brief.md` is still accepted as legacy evidence so in-flight workspaces degrade toward the safe branch. (Final-review fix wave.)
- **2026-07-30:** Cloud sync unwraps the overlay at the `WorkspaceSyncBase` seam instead of listing virtual prefixes in `SYNC_IGNORE_PATTERNS` — structural, so a future provider cannot reintroduce the contacts PII leak or the `VirtualPathError` that silently disabled sync for a whole session. Consequence recorded in Slice 2. (Final-review fix wave.)
