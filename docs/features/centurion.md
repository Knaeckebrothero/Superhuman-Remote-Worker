...
1. **Crash-ended resurrection policy.** A crash-`ended` officer whose flag is still true: page-and-wait (recommended — resurrection of a repeatedly-crashing officer is a loop) vs. auto-resurrect gated on `ended_at` recency. v1 implements page-and-wait unless overruled.
2. **First command & loop conversion timing.** Converting the live RSI project requires its in-flight campaign to conclude or be aborted first (the conversion guard). Alternatives: wait for campaign end; abort and hand the backlog to the centurion; or first-command a smaller project while the RSI campaign finishes. Needs a call when implementation reaches S8.
3. **Page budget default** — 3/day (recommended) with urgent-only quiet-hours bypass; SRE practice argues 2, empirical agent studies 3–5.
