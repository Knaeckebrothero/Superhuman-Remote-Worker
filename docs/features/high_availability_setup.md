...
- [ ] **Migrate dev/homelab — BLOCKED on two gates** (verified live 2026-07-30, see the runbook's "Prerequisites for the Fleet-managed dev/homelab cluster"): (a) **`GITEA_DB_PASSWORD` is absent from Vault** at `homelab/superhuman-remote-worker/srw-secrets` — that cluster is ESO-synced, so the chart's generate-and-preserve fallback does not apply and `srw-giteadb` would fail to start; adding it is an operator/Vault action. (b) **no published chart contains the change** — Fleet pins `0.0.0-dev.sha-576a15f`, and `22c07e5f` was pushed batched under a docs-only tip so CI's change-detection cut no chart; any later `helm/` commit publishes one and auto-bumps `fleet.yaml`. The cluster is safely pinned to `sqlite3` meanwhile. Scale there is ~10× k3d (443 repos, 31 MB), and agents push to Gitea mid-job, so the several-minute Gitea-down window needs a quiet slot.
