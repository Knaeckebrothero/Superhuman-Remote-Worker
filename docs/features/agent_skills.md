...
> **Status**: Slices 1 + 2 **shipped on `develop`** (2026-06-18). Slice 1 = authoring foundation (storage/CRUD/zip import-export/Cockpit editor). Slice 2 = runtime engine (orchestrator resolves the in-scope menu into the frozen blob; fenced Layer-1 menu injection; workspace materialization of skill dirs; the `use_skill` tool) — both halves verified live in the deployed k3d images. Next: Slice 3 (expert bindings + `todo_guide`/`research_guide` migration).
