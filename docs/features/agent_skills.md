...
> **Status**: Slices 1 + 2 + 3 + 4 **shipped on `develop`** (1 + 2: 2026-06-18; 3: 2026-06-19; 4: 2026-06-21). Slice 1 = authoring foundation (storage/CRUD/zip import-export/Cockpit editor). Slice 2 = runtime engine (orchestrator resolves the in-scope menu into the frozen blob; fenced Layer-1 menu injection; workspace materialization of skill dirs; the `use_skill` tool) — verified end-to-end on k3d. Slice 3 = expert bindings + migration: the `skill → binding` field (`before_tool`/phase triggers point at a bundled skill), with `todo_guide` and `research_guide` migrated to bundled skills preserving their enforced/phase bindings — verified in the deployed image under both flag states, incl. the flag-off prod-safety path. **Activation correction 2026-08-03:** continuous full-skill phase injection is retired. `phase_start` injects once per concrete phase; legacy `phase` is a once-only alias; action-specific guidance uses phase/freshness-scoped `before_tool` read gates. **One deliberate regression:** `todo_guide`'s per-family `gpt_oss` variant was dropped (bundled skills have no per-family selection yet; recoverable via a future per-family-skills slice). **Slice 4 (script-bearing skills) ✅ shipped 2026-06-21** — scripts run via the existing shell (`run_command`, `shell_tools` semantics) with **no new grant key**; lite-tier (`virtual`) agents reach a shell through the now-built [[workspace_tier_upgrade]] HITL flow (`request_workspace_upgrade`), and a harness-injected `use_skill` note degrades gracefully when scripts can't yet run. Net-new code was just the `use_skill` note + a bundled `word-count` example; verified end-to-end on k3d (see the Slice 4 entry below).
...
|---|---|---|---|
| `before_tool:<tool>` + `enforce: true` | Tool-gated; the tool refuses until a binding-valid read (optionally phase-scoped and turn-bounded) | The guidance is **mandatory** before an action (a guardrail) | **✅ Shipped — Slice 3; freshness scopes 2026-08-03** (`todo-guide`, `verify-before-done`) |
| `phase_start:strategic\|tactical` | Auto-injected exactly once per concrete phase instance | Guidance must orient the phase before any single action can trigger it | **✅ Once-only semantics 2026-08-03** (`research-guide`) |
| `model_invoked` | Agent picks from the menu (via `use_skill`); see [Activation](#activation--model-invoked-with-deterministic-bindings-preserved) | **Many** possible skills, only some apply — discovery has value | **✅ Shipped — Slice 2** |
...
2. **Expert binding.** An expert's `config` references skills and, per skill, may pin a binding: *none* → model-invoked; `before_tool:<tool>` (+ `enforce`, optional phase/freshness scope) → deterministic/gated; `phase_start:…` → injected once at phase entry.

The skill is written **once**; each expert decides **how it activates**. The same `code_review` skill might be model-invoked for a generalist but `phase_start:strategic`-injected for the `critic`. This is exactly what today's `instruction_files` list expresses (`file → trigger → enforce`); it generalizes to `skill → binding`, and `todo_guide` is already an instance. There is deliberately no continuous full-skill injection mode.
...
- **Script execution is an untrusted-code surface** — Anthropic explicitly warns a malicious skill can direct the agent to misuse tools. Allowing a skill's scripts to *run* is therefore gated by **an existing shell** — `run_command` under the shipped `shell_tools` semantics — with **no new grant key**. An agent on a lite tier (`virtual`, no shell) cannot run scripts at all until it reaches a shell-capable tier through the now-built [[workspace_tier_upgrade]] HITL flow (the agent calls `request_workspace_upgrade`, a human approves, the Sec-1 grant gate runs). So the threat envelope equals *"a fenced, deny-scanned skill body directs a shell-holding agent to run a command"* — already accepted in Slices 1–3 — plus a human-in-the-loop gate for the lite-tier population. Script source is `fence_persona`'d + `hard_deny_scan`'d at save like any fragment.
...
- **Four-layer prompt architecture** ([[prompting]]): Layer 3 *is* the skill artifact, bound deterministically; passive tool-gated reads and once-only phase-start injection are the two bounded delivery mechanisms. This feature adds the `model_invoked` binding + catalog.
...
10. **Security reuses experts' mechanisms** (`fence_persona` for description+body, `hard_deny_scan` at save). **Script execution needs a shell** — `run_command` under the shipped `shell_tools` semantics, **no new grant key**; lite-tier (`virtual`) agents reach a shell via the now-built [[workspace_tier_upgrade]] HITL flow. Script-bearing skills stayed a later slice for **scope/risk** (get prompt-only right first), no longer for missing infra.
...
- **Script-execution gate** — *resolved 2026-06-21:* no new grant key. Skill scripts run via `run_command`/`shell_tools` on a shell tier; lite-tier agents reach one through the now-built [[workspace_tier_upgrade]] HITL flow. Folded into the Slice 4 entry.
...
- **Slice 1 — Authoring foundation. ✅ SHIPPED (`develop`, 2026-06-18).** `SKILLS_DB_ENABLED`; storage (bundled `config/skills/` + project Gitea `skills/` + `skills` row + `skill_files`); the `SKILL.md` parser/serializer; CRUD + duplicate + native-zip import/export + edit; Cockpit editor cloned from the experts editor (`/skills`, `/skills/new`, `/skills/{id}/edit`); `hard_deny_scan` + path-traversal validation at save (persona fencing is a runtime/injection concern → Slice 2). **No agent runtime (that's Slice 2).** Plan + as-built: `docs/superpowers/plans/2026-06-18-skills-slice-1.md`. *DoD met: round-trips a real Claude Code skill (import → edit → export, byte-comparable) and create/edit from scratch via the `/skills` editor.*
- **Slice 2 — Runtime engine. ✅ SHIPPED (`develop`, 2026-06-18).** Resolve the in-scope menu into the resolved blob (`_gather_in_scope_skills` → `resolve_config(skills=…)`); materialize skill dirs into the workspace (`_deploy_instruction_files`); `use_skill` (L2, a workspace tool); fenced Layer-1 menu injection (L1, `fence_skills_menu` + `{available_skills}`). Prompt-only, open catalog, gated by `SKILLS_DB_ENABLED`. Plan + as-built: `docs/superpowers/plans/2026-06-18-skills-slice-2.md`. *DoD met end-to-end on k3d: a real autonomous LLM job (gemma-4) was dispatched, the orchestrator attached the `hello-skill` menu+files to its frozen blob, the agent materialized `skills/hello-skill/SKILL.md` into its workspace, and the agent **called `use_skill("hello-skill")` and received the SKILL.md body** (audit iter 14).*
- **Slice 3 — Expert bindings + migration. ✅ SHIPPED (`develop`, 2026-06-19; activation corrected 2026-08-03).** `InstructionFileEntry` gained an optional `skill:` (XOR `file:`) resolving via `.path` to `skills/<name>/SKILL.md`; gates are satisfied by `read_file` **or** `use_skill`, which record the same path. Bound skills ride the **flag-independent `instructions` blob channel** (frozen by `serialize_resolved_config`, deployed by `_deploy_instruction_files`) so deterministic delivery survives with the catalog off; bound skills are filtered out of the model-invoked menu (`filter_bound_skills`). The original migration bound `research-guide` to `phase:tactical` and `todo-guide` to `before_tool:next_phase_todos`; the activation correction makes the former `phase_start:tactical` and checkpointed once per concrete phase, while all legacy `phase:*` bindings inherit those once-only semantics. **Deliberate regression:** the per-family `todo_guide_gpt_oss.md` variant was dropped — bundled skills have no per-family selection yet. Plan + original as-built: `docs/superpowers/plans/2026-06-19-skills-slice-3.md`. *The original deployed DoD proved flag-independent materialization and read-gate behavior; the 2026-08-03 correction is covered by worker-graph regressions for duplicate suppression, same-phase non-reinjection, later-phase reinjection, and phase/freshness action gates.*
- **Slice 4 — Script-bearing skills. ✅ SHIPPED (`develop`, 2026-06-21).** Makes a skill's bundled `scripts/` runnable, output-only (source never enters context); rides the now-built [[workspace_tier_upgrade]] unchanged. **Gate = an existing shell, no new grant key:** on a shell-capable tier (`sandbox`/`vm`) the agent runs `run_command python skills/<name>/scripts/<x>` under the shipped `shell_tools` semantics — the same way Claude Code runs skill scripts through plain Bash; on a lite tier (`virtual`, no shell) it can't, so it calls the now-built **`request_workspace_upgrade(reason)`** (`src/tools/core/upgrade.py`) → human approves → the existing **`seed_workspace`** copy carries the skill's `scripts/` into the new sandbox (already in the seeded prefix) → the next turn runs them. **Net-new code is the "skills know about the upgrade" seam — not execution, not grants:** (1) detect a script-bearing skill (its files include `scripts/*`); (2) a **harness-injected `use_skill` degradation note** on `supports_shell == False` backends — *"this skill includes runnable scripts; you're on a virtual filesystem — the guidance applies now, call `request_workspace_upgrade` to execute the scripts"* — injected by the harness, not the body, so real Claude Code/Codex skills stay portable; (3) *(deferred, not built)* an optional "runs scripts" marker in the L1 menu — the `use_skill` note already delivers the affordance at load time, so the marker is only a one-turn optimization. Same seam covers **sessions and worker jobs** (both expose the upgrade tool; keyed on `supports_shell`, not job type). **Decline → the skill still works as prose;** only the scripted part is withheld. *DoD met on k3d (2026-06-21).* Net-new code shipped: the `use_skill` degradation note (`src/tools/workspace/skills.py`, fires on `supports_shell == False` when the skill has a `scripts/` dir) + a bundled stdlib-only/output-only `word-count` example (`config/skills/word-count/`); **no grants/orchestrator/upgrade changes**. Evidence: a real autonomous job (`gemma-4-moe`) on the **`virtual`** tier (capability gate dropped `run_command`/shell) materialized `skills/word-count/`, called `use_skill("word-count")`, and — acting on the injected note — called `request_workspace_upgrade` (reason: *"the 'word-count' skill requires running a Python script via a shell, which is not available in the current virtual workspace"*); the `workspace_upgrade_required` freeze drove the in-process **virtual→sandbox** upgrade; the seed carried both the agent's `sample.txt` and `skills/word-count/` into the sandbox; `run_command` reloaded; the agent ran `python skills/word-count/scripts/wordcount.py sample.txt` → **`lines=2 words=9 chars=44`**, recorded in `output/word_count_report.md`. This drove the **worker auto-upgrade** path; the **session** human-approval surface (cockpit *Upgrade workspace* → `upgrade-to-workspace`) was already k3d-verified when [[workspace_tier_upgrade]] shipped, so the previously-unproven seam — the note + bundled skill correctly triggering that upgrade, then the script running on the seeded sandbox — is now proven. Plan + as-built: `docs/superpowers/plans/2026-06-21-skills-slice-4.md`.
- **Later — Per-family skill variants (concept captured 2026-06-19; deferred until the core skills slices are done).** A skill carries family-specific bodies/files (e.g. a gpt-oss-tuned `todo-guide`), resolved by the same `model_family` mechanism experts already use for per-family prompts/instructions (`MatrixResolver` + `model_config_matrix`). **Motivation:** restore the `todo_guide_gpt_oss.md` variant dropped in Slice 3, plus the general capability. **Scope (decided 2026-06-19): full** — bundled + DB-authored skills + Cockpit editor authoring. **Key consideration — *not* a drop-in of the prompt matrix:** a skill is a *directory*, not a single file, so resolution differs, and it must stay import/export-portable with Claude Code/Codex (variants degrade to the base `SKILL.md`). **Leading candidate (encoding TBD at design time):** per-family file overrides inside the skill dir — a canonical `SKILL.md` plus optional `SKILL.<family>.md` siblings, resolved orchestrator-side at delivery so the workspace gets one family-resolved `SKILL.md`; no `skill_files` schema change; mirrors the existing `_gpt_oss` prompt-file convention. *Alternatives weighed:* whole-directory `variants/<family>/`; separate per-family skills selected by matrix name-resolution. **Sequencing:** design via brainstorming → spec → plan once the core skills feature is finished; until then gpt_oss-family agents get the default `todo-guide`.
