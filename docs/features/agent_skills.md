...
> **Status**: Slices 1 + 2 + 3 **shipped on `develop`** (1 + 2: 2026-06-18; 3: 2026-06-19). Slice 1 = authoring foundation (storage/CRUD/zip import-export/Cockpit editor). Slice 2 = runtime engine (orchestrator resolves the in-scope menu into the frozen blob; fenced Layer-1 menu injection; workspace materialization of skill dirs; the `use_skill` tool) — verified end-to-end on k3d. Slice 3 = expert bindings + migration: the `skill → binding` field (`before_tool`/`phase` triggers now point at a bundled skill), with `todo_guide` and `research_guide` migrated to bundled skills preserving their enforced/phase bindings — verified in the deployed image under both flag states, incl. the flag-off prod-safety path. **One deliberate regression:** `todo_guide`'s per-family `gpt_oss` variant was dropped (bundled skills have no per-family selection yet; recoverable via a future per-family-skills slice). **Next: Slice 4 (script-bearing skills)** — design settled 2026-06-21: scripts run via the existing shell (`run_command`, `shell_tools` semantics) with **no new grant key**; lite-tier (`virtual`) agents reach a shell through the now-built [[workspace_tier_upgrade]] HITL flow (`request_workspace_upgrade`), and a harness-injected `use_skill` note degrades gracefully when scripts can't yet run (see the Slice 4 entry below).
