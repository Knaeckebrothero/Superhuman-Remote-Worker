...
|---|---|---|---|
| `before_tool:<tool>` + `enforce: true` | Tool-gated; the tool refuses until the skill is read | The guidance is **mandatory** before an action (a guardrail) | **✅ Shipped — Slice 3** (`todo-guide` skill) |
| `phase:strategic\|tactical` | Auto-injected on phase transition | One skill always applies during a phase | **✅ Shipped — Slice 3** (`research-guide` skill) |
...
- **Slice 3 — Expert bindings + migration. ✅ SHIPPED (`develop`, 2026-06-19).** `InstructionFileEntry` gains an optional `skill:` (XOR `file:`) resolving via `.path` to `skills/<name>/SKILL.md`, so the shipped `before_tool`/`phase` enforcement reuses unchanged (the gate is satisfied by `read_file` **or** `use_skill`, which records the same path). Bound skills ride the **flag-independent `instructions` blob channel** (frozen by `serialize_resolved_config`, deployed by `_deploy_instruction_files`) so deterministic enforcement survives with the catalog off (prod); bound skills are filtered out of the model-invoked menu (`filter_bound_skills`). Migrated `research_guide` → bundled `research-guide` (`phase:tactical`) and `todo_guide` → bundled `todo-guide` (`before_tool:next_phase_todos`), removing the bespoke instruction-matrix special-case (HARDCODED_DEFAULTS + the `agent.py` deploy block + 4 `model_config_matrix` entries) and repointing 6 strategic-todo templates. **Deliberate regression:** the per-family `todo_guide_gpt_oss.md` variant was dropped — bundled skills have no per-family selection yet (recoverable via a future per-family-skills slice). Plan + as-built: `docs/superpowers/plans/2026-06-19-skills-slice-3.md`. *DoD met in the deployed image under both flag states: scholar binds both guides as skills, both froze into the `instructions` channel with the catalog off, the `next_phase_todos` gate targets `skills/todo-guide/SKILL.md` and opens on read, the tactical phase injects `skills/research-guide/SKILL.md`, and flag-on filtering strips both bound guides from the `use_skill` menu. Also confirmed in a full autonomous k3d run (gemma-4-moe): a live job's `next_phase_todos` was nudged with "you must read `skills/todo-guide/SKILL.md`", the agent read the 14.8k-char skill body, and the retried call returned "Staged 5 todos" — the migrated gate opened on read.*
