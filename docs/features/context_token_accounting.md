...
### 4.4 Image-safe recovery (S3 — confirmed **live/unimplemented**)
...
3. **S3 — Image-safe recovery.** Strip image blocks from the three summarizer-input arms; teach elision to shed image messages (at compaction only); fix the timeout log. *Confirmed unimplemented — the bug is live. Makes genuine overflows recoverable.*
