...
> **Status**: Research + recommendation, 2026-06-21. **First build shipped 2026-06-22: `verify-before-done`** — authored as a bundled skill (`config/skills/verify-before-done/`) and bound `phase:tactical` on the worker experts (`defaults` → developer/critic/curator/bughunter, plus the overriders `scholar` + `designer`). Its stronger enforcement tiers (**B** read-gate, **C** trace-gate) are deferred follow-ups — see [Enforcement model & follow-ups](#enforcement-model--follow-ups). Defines which bundled **skills** SRW should ship *by default in every deployment*, structured as a tight always-on **universal core** plus **opt-in** and **expert-dedicated** tiers. Builds on the shipped skills substrate ([[agent_skills]]) and is the skills-side companion to [[default_expert_roster]]. **Tier-1 core is complete** (2026-06-24): `verify-before-done` + `brainstorming` + `systematic-debugging` all built (`planning-and-decomposition` already shipped as `todo-guide`, optimized 2026-06-24); `brainstorming` and `systematic-debugging` are model-invoked (catalog), `verify-before-done` is bound `phase:tactical`. **Tier-2 started** (2026-06-25): `code-review` **built** — `config/skills/code-review/SKILL.md`, shipped **model-invoked** (catalog), evidence base at [`../../researches/code-review.report.md`](../../researches/code-review.report.md). Open: an optional `critic` `phase:tactical` binding (reviewing is the critic's core job) — deferred pending the deep-merge re-list cost (the critic inherits all four defaults bindings and an `instruction_files` block would replace, not extend, that list). **`project-onboarding` **built** (2026-06-25)** — the old `codebase-onboarding` entry, **reframed** off "codebase" because SRW's unit of work is a *project* (a datasource *bundle*: cloud/vault/shared-folder files, databases, *and* optionally a repo, plus an accumulated KB) — code is one branch, not the skeleton. `config/skills/project-onboarding/SKILL.md`, shipped **model-invoked** (universal), evidence base at [`../../researches/project-onboarding.report.md`](../../researches/project-onboarding.report.md). **Transfer verdict came back stronger than debugging's: universal *as a procedure*** (the inventory → source-of-truth → structure/vocabulary → map skeleton is the named procedure in archival survey, *First 90 Days*, consulting, journalism, Klein's data-frame sensemaking), so one body serves all experts. Load-bearing halves: orientation is the bottleneck (comprehension ~58% of dev time, Xia TSE 2018; MAST FC1 41.8%) **and** it must terminate (the explore-forever failure is MAST FM-1.5; stop when you can write the map + a located plan). **`test-driven-development` **built** (2026-06-25)** — `config/skills/test-driven-development/SKILL.md`, shipped **model-invoked, CODE-SCOPED** (the `systematic-debugging` pattern: a code-shaped description self-scopes), evidence base at [`../../researches/test-driven-development.report.md`](../../researches/test-driven-development.report.md). **Transfer verdict is the weakest of the roster — universal only as a *principle* (pre-registration is the rigorous non-code analogue, 96%→44% positive-result drop), code-specific as a *procedure*, and agent practice is near-totally code** — so it ships code-scoped, not universal, carrying one sentence that names the principle. Built on the **evidence-backed core, not the dogma**: Fucci's dissection (IEEE TSE 2017) shows test-*first* ordering "had no important influence" — the active ingredient is *small uniform verified increments* + an executable up-front check + a regression net; strict ordering and 100% coverage are weakly-supported dogma (meta-analysis quality effect small/NS, *negative* vs. incremental test-last). The agent-specific teeth are the watch-it-fail-first / don't-edit-the-test guardrails — documented defense against agents *gaming* tests (`__eq__`→True, `sys.exit(0)`, editing the test file; ImpossibleBench) — closing MAST FM-3.2+3.3 (~17%). Open: optional `developer` `phase:tactical` binding, deferred with code-review→critic.
>
...
| Skill | Body covers | Validation | SRW mapping |
|---|---|---|---|
| **`test-driven-development`** | Failing test first → minimal code → green → refactor, in *small verified increments* (the evidence-backed core; not 100% coverage, not the test-first ritual); watch-it-fail-first + don't-edit-the-test guard against gaming | Cursor + Anthropic CC best-practices ("favorite" agentic workflow) + Cline + superpowers; Fucci IEEE TSE 2017 (active ingredient) | **built** model-invoked, **code-scoped** (self-scopes like `systematic-debugging`); home = `developer` |
| **`code-review`** (giving) | Review a diff for bugs / security / performance; structured findings | superpowers `requesting-code-review` + Cline `code-review.md` + Anthropic plugin | **built** model-invoked (universal); home = `critic` |
| **`project-onboarding`** (was `codebase-onboarding`) | Orient in an unfamiliar *project* (datasource bundle: cloud/vault/folder files, DB, optionally a repo) before acting: inventory → confirm the source of truth → learn structure/vocabulary → write a reusable map → stop | Cline `codebase-onboarding.md` + `/onboard` skill + Cline Memory Bank + Aider repo-map; non-code: archival survey / *First 90 Days* / consulting / Klein | **built** model-invoked (**universal** — code is one branch) |
...
These are heavily validated but **dev/coordination-leaning** rather than truly universal, so they are off-by-default for a knowledge-work/home deployment and on for a dev shop. (The Tier-2-vs-Tier-3 boundary for TDD and code-review is a judgment call — see [Open decisions](#open-decisions).) **Exception: `project-onboarding` is genuinely universal** once reframed off "codebase" — orienting in an unfamiliar body of work applies to a document corpus / vault / dataset as much as a repo, which is why it ships model-invoked for all experts rather than dev-bound.
...
- **reading/navigating large codebases** → captured as Tier-2 `project-onboarding` (built 2026-06-25; reframed from `codebase-onboarding` to cover any datasource bundle, not just a repo).
