...
> **Status**: Research + recommendation (2026-06-21), now in **build-out**. **Tier-1 universal core complete** (2026-06-24); **Tier-2 is 3 of 4 built** (2026-06-25). For the live snapshot — what's built, the scope/binding decisions, the per-skill pipeline, the open binding decision, and what's next — see **[Build status & resume state](#build-status--resume-state)** immediately below. This doc defines which bundled **skills** SRW ships *by default in every deployment*, structured as a tight always-on **universal core** plus **opt-in** and **expert-dedicated** tiers. Builds on the shipped skills substrate ([[agent_skills]]) and is the skills-side companion to [[default_expert_roster]].
>
...
### The transfer-verdict spectrum (the cross-skill through-line)

The recurring intellectual question each build answers is *"how far does this code-validated skill generalize to knowledge work?"* — the roster's central unmeasured premise. The answers form a spectrum, and each skill's **scope decision falls out of where it lands**:

1. **Universal as a procedure** → ships universal, one body, all experts: `project-onboarding`.
2. **Universal as a frame** → one body all experts, but load-bearing steps stay code-shaped: `code-review`.
3. **Universal only as a principle** → ships code-scoped, one sentence names the principle: `test-driven-development`.
4. **Code-scoped, no real transfer** → code-scoped, self-scoping description: `systematic-debugging`.

The honest move throughout: **build on the evidence-backed core, not the dogma**, and don't inflate a weak transfer (TDD's Fucci finding — small uniform increments, not the test-first ritual; debugging's RCA bridge failing its own evidence test).
...
- **model-invoked = UNBOUND.** A skill stays in the catalog iff no expert's `instruction_files` names it. `filter_bound_skills` (`src/core/skill_resolution.py`) strips only bound skills; `_scan_skills` (`orchestrator/main.py`) globs `config/skills/*/SKILL.md`.
- **The `deep_merge` REPLACE trap** is why home-expert bindings are deferred: an expert's `instruction_files` **replaces** (not extends) the inherited 4-entry defaults list, so binding a skill there forces re-listing all inherited bindings, which then **drift** from defaults.
- **MCP `orchestrator-cluster` → `mcp.superhuman-remote-worker.com` = the DEV cluster, NOT local k3d.** So `list_skills`/`reload_skills` reflect dev (which lacks uncommitted skills). Always verify locally via the kubectl parse-in-pod above, not the MCP catalog.
