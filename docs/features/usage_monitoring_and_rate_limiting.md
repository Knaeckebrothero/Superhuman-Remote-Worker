...
| 5a | Upstream backstop | Set per-deployment `rpm`/`tpm` in LiteLLM `config.yaml` to each provider key's **real capacity** — not a user limit, the upstream's physical truth — so Σ(per-user limits) can't blow the shared provider ceiling. |
