...
- **2026-06-01 — Reasoning-default toggle (realizes the Slice 1 #3 alternative).** Slice 1 #3 hard-coded reasoning cards to always-open (`#thoughtCard` `<details open>`) but explicitly left the door open for *"a 'collapse reasoning by default' toggle in session settings."* Added that toggle on user request. It's a **display-only** preference — it changes nothing the agent sees, only how the client renders reasoning the agent already produced.
  - **New service `ChatPreferencesService`** (`core/services/chat-preferences.service.ts`, + spec): a `reasoningExpanded` signal backed by **localStorage** (`cockpit:chat:reasoningExpanded`, default `true` = the historical behaviour), `setReasoningExpanded()`, SSR-safe. Modelled on `ThemeService` and stored **per-device, not backend-persisted**, for the same reason: it's a viewing choice (expanded on a wide desktop, folded on a phone) and must apply at first paint. *Not* in `SettingsService.preferences` (that's the REST-backed `UserSettings`) and *not* in the thread's `config_override` (that's agent config).
  - **Wiring:** `#thoughtCard`'s `<details>` `open` is now bound `[attr.open]="chatPrefs.reasoningExpanded() ? '' : null"` (was a bare `open`) — covers inline thought cards *and* those inside tool groups (both use the one `#thoughtCard` template). New settings-panel row (`app-select` Expanded/Collapsed) between **Narration** and **Model**, with `onReasoningDefaultChange()`. **No SCSS** (reuses `.settings-row`/`app-select`) → component stylesheet stays 35.40 kB. New i18n `chat.settings.reasoning` / `reasoningExpanded` / `reasoningCollapsed` (EN + de-DE; "Reasoning" kept as the loanword in DE to match the rest of the app). Flipping the signal **re-applies live** to all visible cards (it's an attribute binding), and the user can still fold/unfold an individual card by hand.
  - **Note on reachability:** the settings panel is gated behind `@if (chat.isConnected())` (live sessions only) — so, like Mode/Model/Temperature, the toggle is changed *during* a session, but the preference is device-global and persists, so it then governs every session incl. historical views.
  - **Verified (unit/build):** `chat-preferences.service.spec.ts` 6/6 (default true, read true/false, malformed→default, persist, cross-instance round-trip); full cockpit suite **438/438**; clean AOT `npm run build` (no errors; budget unchanged). *(A transient `ng serve` TS2339 overlay flashed during Tilt's multi-edit sync — the AOT build is clean and the runtime worked throughout; it was a dev-server incremental-compile race, not a real error.)*
  - **Verified (live on local k3d, Playwright, 2026-06-01).** On the seeded thread `aaaaaaaa-…-0001` (has reasoning cards): settings panel shows the **Reasoning** row (order: Mode · Narration · Reasoning · Model · Temperature) defaulting to **Expanded**; expanded reasoning cards render `open`. Driving the native `<select>` (real `change` → `onReasoningDefaultChange`) to **Collapsed** closed both visible thought cards **live** and wrote `localStorage=false`. **Full reload** → the cards render **closed by default** and the select **reflects** the persisted `collapsed`. Flipping back to **Expanded** re-opened them live + `localStorage=true` (default restored). Screenshot `reasoning-setting-panel.png` (repo root). Changes uncommitted.

- **2026-06-01 — Tool-call grouping reworked: cornerless + a Tool calls display toggle.** Two coupled changes on top of the #10 grouping, on user request: (a) the folded "N× tool calls" disclosure was a **box** (`border:1px` + radius + `overflow:hidden`); made it **cornerless** — `border-left:2px` accent + `radius-tag`, mirroring the reasoning `.thinking-block` so the two foldable affordances read alike. (b) New **Tool calls** display setting (Expanded/Collapsed, parallel to Reasoning) — when **Expanded**, *every* run renders as plain inline cards with **no group wrapper and no fold control** ("like a 2-call run"); when **Collapsed** (default — preserves the Slice 3 clutter-reduction), runs of 4+ fold into the cornerless disclosure. User chose **Folded by default**.
  - **Mechanics:** added `toolCallsExpanded` to `ChatPreferencesService` (localStorage `cockpit:chat:toolCallsExpanded`, default `false`). New pure exported helper **`shouldFoldToolRun(count, expanded, threshold)`** = `!expanded && count >= threshold`; the old `isLongToolRun` method was replaced by `foldToolRun(tools)` (delegates to the helper, reads the pref). Template `@if (foldToolRun(group.tools))` → folded disclosure; `@else` → inline cards (the existing short-run branch, now also used for long runs when expanded). New settings row + `onToolCallsDefaultChange`; new i18n `chat.settings.toolCalls/toolCallsExpanded/toolCallsCollapsed` (EN+DE). **Render-only**; SCSS effectively unchanged (35.41 kB — restyle swapped properties, didn't grow).
  - **Verified (unit/build):** `+4` `shouldFoldToolRun` cases (folds long-run-when-collapsed, short-run-always-inline, never-folds-when-expanded incl. 50 calls, inclusive-threshold) + `+3` service cases (toolCallsExpanded default false / read / independent persist). Full cockpit suite **445/445**; clean AOT `npm run build` (no dangling `isLongToolRun`; budget unchanged).
  - **Verified (live on local k3d, Playwright, 2026-06-01).** Seeded thread `aaaaaaaa-…-0001` (5 long runs + short runs). **Default (Collapsed):** 5 cornerless folded "N× tool calls" disclosures — computed border `{top:0, right:0, bottom:0, left:2px}` (box gone, left accent only); short runs inline. Setting **Tool calls → Expanded** → `details.tool-group` count **5 → 0**, inline `.event-tool` cards **2 → 27** (every call inline, no fold control, incl. the denied `run_command`). `localStorage=true`. **Full reload** → still inline (groups 0, cards 27), select **reflects** `expanded`. Flip back to **Collapsed** → 5 folded groups return, `localStorage=false` (default restored). Screenshots `toolcalls-folded-cornerless.png` + `toolcalls-expanded-inline.png` (repo root). 0 feature console errors. Changes uncommitted.

- **2026-06-03 — Compaction summaries now render markdown.** The `/compact` (or auto-compaction) "View summary" disclosure showed the agent's summary via plain `{{ turn.summary }}` interpolation — so headings/lists/code in it appeared as literal `##`/`-`/backticks. Now renders `<markdown [data]="turn.summary">`. **Zero new CSS:** the body div gets the `message-body` class, reusing the chat's whole markdown `::ng-deep` cascade (code blocks + Prism theme, lists, tables, links, blockquotes) — `.message-body`'s base rule is just font/line-height/min-width; the bubble styling is gated on `.message-user`/`.message-assistant`, which this isn't. SCSS: dropped `.compaction-summary-body { white-space: pre-wrap }` (markdown block elements structure it; pre-wrap would turn inter-block newlines into gaps) and pinned `font-size: 12px` so `.message-body`'s 13px doesn't enlarge the muted summary.
  - **Verified (authoritative, local):** clean AOT `npm run build` (template type-checks; budget unchanged) + full suite **451/451**. Render-only, no new pure logic, so no new unit test (consistent with the #8 `finalAnswer` markdown change). Reuses the exact `<markdown>` mechanism already proven live this session for the final answer, text events, and thought cards.
  - **Verified (live on local k3d, Playwright, 2026-06-08).** (The live check was deferred several days while the local cluster was down — `tilt up` had stopped → stale pod → kubeconfig context lost → k3d API unreachable; once the user brought the loop back, Tilt had synced the change and ng serve compiled clean.) Opened thread `aaaaaaaa-…-0001` (the seeded `role='summary'` row persisted in the postgres PVC) and expanded "View summary". DOM-asserted the body **renders markdown, not literal text**: `message-body` class present; `<h2>` "Conversation summary" (no literal `##`), `<strong>`, `<ul><li>`, inline `<code>`, a `<pre>` fenced block **with Prism `.token` syntax highlighting** — all from the reused `.message-body` cascade; `hasLiteralHashes:false`, `hasLiteralBullets:false`. Screenshot `compaction-summary-markdown.png` (repo root) shows the heading, bold/italic, muted inline-code chips, bulleted list, and the syntax-coloured `python` block inside the cornerless left-accent border. Changes uncommitted.
