...
- **2026-06-03 — Compaction summaries now render markdown.** The `/compact` (or auto-compaction) "View summary" disclosure showed the agent's summary via plain `{{ turn.summary }}` interpolation — so headings/lists/code in it appeared as literal `##`/`-`/backticks. Now renders `<markdown [data]="turn.summary">`. **Zero new CSS:** the body div gets the `message-body` class, reusing the chat's whole markdown `::ng-deep` cascade (code blocks + Prism theme, lists, tables, links, blockquotes) — `.message-body`'s base rule is just font/line-height/min-width; the bubble styling is gated on `.message-user`/`.message-assistant`, which this isn't. SCSS: dropped `.compaction-summary-body { white-space: pre-wrap }` (markdown block elements structure it; pre-wrap would turn inter-block newlines into gaps) and pinned `font-size: 12px` so `.message-body`'s 13px doesn't enlarge the muted summary.
  ...
  - **Live k3d verification DEFERRED — local dev cluster is down.** Across a multi-day gap `tilt up` had stopped → the cockpit pod went stale → kubeconfig context was lost → the k3d API server became unreachable (`0.0.0.0:44317` refused). Bringing the full dev loop back (cluster start + pods + `tilt up` resync/rebuild + secrets per the README traps) is out of scope for this change and is user-driven. A `role='summary'` row with rich markdown (h2 + bold/italic + inline code + bulleted list + fenced `python` block) was **seeded** into thread `aaaaaaaa-…-0001` (`/tmp/seed_compaction_summary.sql`, idempotent) and persists in the postgres PVC, so the live check is one Playwright pass away once the loop is back: open the thread → expand "View summary" → assert `<h2>`/`<ul>`/`<pre>` render (not literal `##`/`-`/```). Changes uncommitted.
