...
**Status:** Slices 1 & 2 ✅ shipped + verified (local k3d + live on the dev cluster); #8's collapsed-turn view was reworked 2026-06-01 to keep the full final answer visible (see decision log). Slice 3 ✅ shipped + verified (local k3d render logic; deploy confirmed live on the dev cluster — running bundle serves the grouping code, SW not stale). Slices 4–6 and the separate workstreams remain backlog. See the decision log for per-item detail.
**Filed:** 2026-05-20
