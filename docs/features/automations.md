...
**Status:** The cron half (v0) **shipped + polished 2026-05-18 + 2026-05-20** per the focused implementation cut in [`automations_v0.md`](./automations_v0.md) — backend, cockpit, tests, and external-integration docs all landed and were live-verified on the experimental cluster. The 2026-05-20 polish sweep closed three follow-ups (project-scoped list view, auto-disable notification via SSE + email, cockpit unit-test coverage); only the `/api/automations/{id}/preview` endpoint remains deferred to v0.5. Event triggers + per-chain cost cap + outbound webhooks remain on the v0.5 / v1 roadmap as originally scoped below.
...
> **Correction (2026-06-13).** This "for free" property was *aspirational, not automatic*. `create_job_from_automation` originally called `db.create_job()` (the bare INSERT) directly — it did **not** go through the `POST /api/jobs` *handler*, where Gitea repo/branch creation + the creator access-grant were open-coded. So automation-spawned jobs silently got **no workspace repo**, and the cockpit Workspace button 404'd (reported by students; 0/11 automation jobs had a repo vs 40/40 manual). Fixed by extracting that provisioning into `orchestrator/services/job_provisioning.py::provision_job_repo(...)` and calling it from the handler **and** both automation paths (cron dispatcher + run-now). The "indistinguishable from a manual job" property now actually holds. **Lesson: provisioning is a side effect of *the handler*, not of `db.create_job()` — any new job-creation path that calls `db.create_job()` directly must also call `provision_job_repo(...)`.**
...
- **`create_job_from_automation` creates the job via `db.create_job()` then provisions it via the shared `provision_job_repo(...)`** — the same insert + Gitea provisioning the `POST /api/jobs` handler runs (provisioning was *not* shared until the 2026-06-13 extraction; see the Key-property correction above). The created job carries `context.automation_id = <automation.id>`, `context.trigger = 'cron'`, and `context.scheduled_for = <iso8601>` so it's filterable in the job list and audit-visible. The new job's `priority` is populated from `r["priority"]` — the existing two-phase auto-assign dispatcher (`orchestrator/main.py:2146`) handles ordering and preemption with no scheduler-side changes.
