...
**S3 implementation update (2026-08-08, superseding the S3 status in the v3.2 paragraph above):** Gates 1 and 2 are implemented and k3d-verified on `feature/stateless-workers-s3`; worker admission remains off. S3 added the jobs-side coexistence partition, shared freeze taxonomy, unknown-loop-freeze fail-soft behavior, and a default-unarmed wall-clock batch boundary. Gate 3 disproved the design's assumed assigned-agent completion CAS, so no worker driver or traffic was enabled. The requested one-Deployment experiment is a no-ship until durable completion finalization and an enforceable availability reservation exist (§5.4.4, §5.8, §9.1).
...
Written against the code on `feature/stateless-agents` plus the S3 safety work
on `feature/stateless-workers-s3`, not against intent. The short version: **the
shared substrate is built and genuinely kind-agnostic; the session driver
exists; S3 Gates 1 and 2 are built, but Gate 3 disproved a design assumption and
worker admission remains off; most of S1's surround does not exist.**
