...
0. **Prep (separate small commits, can push before the PR).** Commit this doc;
   push the pending develop commit; add `scripts/verify_ast_equiv.py` + the
   codemod script + LibCST fixtures; run the checkpoint pre-flight inventory
   (§ Checkpoint Compatibility); install `requirements-dev.txt` into `.venv`
   so the local gate equals CI; record the baseline (§ Baseline).
...
`docker/Dockerfile.mcp{,.dev}` (flatten+graft→package, CMD); helm
