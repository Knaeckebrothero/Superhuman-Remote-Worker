...
- **Legacy `mongodb.py` `MongoDB` store** still carries its own (now-unreachable) `get_*_bulk` methods. The retired Mongo reader isn't used by any endpoint; left untouched to keep this change surgical — remove with the rest of that dead module.
