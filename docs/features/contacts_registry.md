...
**Status:** Design approved 2026-07-26→28 (brainstorm; Cockpit layout picked via visual mockups), spec written 2026-07-29. §Agent surface amended 2026-07-30 to ride on [[virtual_directories]] instead of materialized files.

**Agent surface COMPLETE 2026-08-01.** The `ContactsProvider` shipped with [[virtual_directories]] Slice 1 and is deployed + live-gated on dev (agent `sha-f41970a`): an agent on a project reads `contacts/README.md` and `contacts/<slug>.md` through its file tools, served live from the DB via an internal endpoint that derives the project **server-side** from job/thread identity. Verified on dev with an empty project (renders "No contacts are linked to this project.") and confirmed absent from the workspace's real filesystem. `src/core/contact_files.py` is no longer the seam — the provider renders directly; the helper is dormant/test-only. **Still unverified on the agent path:** a project with contacts actually populating `contacts/<slug>.md`, and the mid-session freshness window — see `tests/virtual_directories_test_coverage.md` §2.1.

**Implemented on `develop` 2026-07-30 (since pushed), except the agent surface (now complete, above):** migration `0076_contacts_normalize.sql` (renumbered twice under concurrent work: the spec said 0072, it shipped as 0075, then moved to 0076 when another session's `0075_project_loop_officer_scheduling.sql` landed and was already applied on dev — `migrate.py` hard-fails the whole runner on duplicate prefixes), DB layer, `orchestrator/routers/contacts.py`, the `send_agent_message` rewire + legacy retirement, and the Cockpit `/contacts` page all landed and passed review. The **agent surface is not implemented** — a materialized-files version shipped and was reverted the same day (`revert(agent): drop contacts materialization…`) when this section was re-based onto [[virtual_directories]]; it now waits on the ContactsProvider. `src/core/contact_files.py` is retained, dormant, as that provider's renderer.

**Live gate RUN on the dev cluster 2026-08-01** (k3d is unusable on the dev machine, so dev is the gate). Migration `0076` applied `success=true`, zero dirty rows, both orchestrator replicas healthy. Exercised against the deployed image: all four `resolve_contact` branches, both opt-in defaults, duplicate→`None`, auto-primary, promotion+demotion, opt-in reset on address edit, all three `list_contacts_for_user` filters, link/unlink, and cascade delete. Cockpit verified end-to-end in a browser: create with two channels, address normalization (`UI-Gate@Example.ORG`→lowercased, `+49 170 555-(0)77`→`+49170555077`), chip annotation `whatsapp·opt-in pending`, read-only expansion, channel select disabled on saved rows, delete dialog naming the affected project.

**One real bug found and fixed:** `update_contact` raised `AmbiguousParameterError` at PREPARE time on *every* call — `$3` appeared only in `IS NULL` / `NULLIF($3, '')`, neither of which pins a type, and asyncpg prepares untyped. **Every `PATCH /api/contacts/{id}` returned 500**, so editing a contact was impossible. Fixed with `$3::text` (`fix(db): cast $3 in update_contact`) plus the integration test that was missing — `update_contact` was one of the six DB methods the reviews flagged as untested, and it was the one that broke. Diagnosis trap worth knowing: the browser reports the 500 as a **CORS error** (`No 'Access-Control-Allow-Origin' header`), because unhandled-500 responses bypass the CORS middleware — the failure looks like misconfigured CORS from the frontend alone.
