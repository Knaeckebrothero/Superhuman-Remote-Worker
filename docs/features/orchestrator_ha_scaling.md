...
**Status (2026-06-26):** **M0 (active-passive hardening) and M1 (leader election) are shipped on `origin/develop`, and M1 is now LIVE-verified on the dev cluster under real traffic** — `replicas: 2` is correctness-safe (leader election + DB guards) and runs on dev today (Fleet overlay, soaking). The live two-replica run validated graceful + hard failover with zero serving blackout and an in-flight job surviving three kills, and caught + fixed two runtime bugs (uvicorn graceful-shutdown hang; IMAP poller log-spam) — see `docs/tests/orchestrator_m1_leader_election_verification.md`. **Remaining before the orchestrator stops being a hard SPOF in production: the deliberate chart-default `replicas: 2` flip (`helm/values.yaml`, Phase 5 / M4) after the dev soak.** M2+ (scale-out, data-tier HA) is still ahead. See the reality-check matrix below.
**Filed:** 2026-05-12
**Refreshed:** 2026-06-24 — reconciled against current code (`main.py` grew ~15.5k → ~23.7k lines; deployment moved to Helm/Fleet; WS proxy removed; several coordination primitives shipped). All line references in this revision are current as of the refresh. **2026-06-26:** M0+M1 implemented, pushed, and k3d-verified (the matrix rows + Phase-0/1 checklists below are updated).
...
| **Layer 1** leader election for singleton loops | not started | **CODE COMPLETE + k3d-VERIFIED (2026-06-26) — `replicas:2` correctness-safe; only the live (dev) run + the replica-count flip remain** | `services/leader_election.py` (single session-scoped lock + `run_when_leader`), lifespan loop-gating, dispatcher CAS claim, Postgres keepalive tuning, **and Task 5 defense-in-depth** (imap claim-table, delegation/digest/notify DB guards) — **pushed to `origin/develop` (2026-06-26)**, **16 PostgresContainer tests green**. **Two-replica failover verified on local k3d** (one leader, gated loops leader-only, failover ×2, graceful step-down) — the run also caught + fixed a deploy-blocking flattened-image import bug (`from orchestrator.…` → top-level imports). Verification: `docs/tests/orchestrator_m1_leader_election_verification.md`. **Remaining:** live (dev) run + the `replicas: 2` flip (Phase 5 / M4). |
...
- **Hard blocker — RESOLVED (M1, on `origin/develop` 2026-06-26; k3d-verified):** the chart still ships `replicas: 1` as the default, but the correctness bugs that made `replicas: 2` unsafe (dispatch double-assign, IMAP double-poll, and the other non-idempotent loops) are now leader-gated + DB-guarded, and the two-replica election/failover wiring is verified on local k3d. `replicas: 2` is correctness-safe; the remaining gates before README/marketing may imply a multi-replica orchestrator are the **live (dev)** two-replica failover run + actually flipping the default count (Phase 5 / M4). Until then, keep the default at 1.
...
- ~~IMAP double-poll / dispatch double-assign if anyone sets `replicas: 2`~~ — **fixed by M1 (2026-06-25):** the loops are leader-gated and the dispatch/imap/delegation/digest/notify paths are DB-guarded. The default stays `replicas: 1` until the live two-replica run + the count flip; the foot-gun is no longer a *correctness* one.
