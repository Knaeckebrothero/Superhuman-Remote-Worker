...
| **Layer 1** leader election for singleton loops | not started | **CODE COMPLETE (2026-06-25) — `replicas:2` correctness-safe; only the live two-replica run + the replica-count flip remain** | `services/leader_election.py` (single session-scoped lock + `run_when_leader`), lifespan loop-gating, dispatcher CAS claim, Postgres keepalive tuning, **and Task 5 defense-in-depth** (imap claim-table, delegation/digest/notify DB guards) — all committed on `develop` (unpushed), **16 PostgresContainer tests green**. Built on `docs/researches/orchestrator_leader_election.md`. Verification: `docs/tests/orchestrator_m1_leader_election_verification.md` (unit PASS; two-replica k3d/live run deferred — cluster cold). |
