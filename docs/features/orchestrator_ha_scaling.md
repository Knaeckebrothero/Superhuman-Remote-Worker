...
**Status (2026-06-26):** **M0 (active-passive hardening) and M1 (leader election) are both shipped on `origin/develop` and k3d-verified** — `replicas: 2` is now correctness-safe (leader election + DB guards), with M1's two-replica election/failover wiring verified on local k3d. Remaining before the orchestrator stops being a hard SPOF in production: the live (dev) two-replica run + the deliberate `replicas: 2` flip (Phase 5 / M4). M2+ (scale-out, data-tier HA) is still ahead. See the reality-check matrix below.
**Filed:** 2026-05-12
...
- [x] **Unlock:** `replicas: 2` is **correctness-safe** (the singleton loops are leader-gated and every correctness-critical path is DB-guarded). Flipping the replica count to 2 + the live two-replica failover run remain (Phase 5 / the verification doc's deferred section).
