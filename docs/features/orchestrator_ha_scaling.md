...
**Status (2026-06-26):** **M0 (active-passive hardening) and M1 (leader election) are shipped on `origin/develop`, and M1 is now LIVE-verified on the dev cluster under real traffic** — `replicas: 2` is correctness-safe (leader election + DB guards) and runs on dev today (Fleet overlay, soaking). The live two-replica run validated graceful + hard failover with zero serving blackout and an in-flight job surviving three kills, and caught + fixed two runtime bugs (uvicorn graceful-shutdown hang; IMAP poller log-spam) — see `docs/tests/orchestrator_m1_leader_election_verification.md`. **The chart default is now `replicas: 2` + `pdb.minAvailable: 1`** (flipped 2026-06-26, M4/Phase-5 declare-done) — the orchestrator is multi-replica HA-by-default and no longer a hard SPOF; dev continues to soak. M2+ (scale-out, data-tier HA) is still ahead. See the reality-check matrix below.
**Filed:** 2026-05-12
