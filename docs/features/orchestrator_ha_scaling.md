...
| **Layer 1** leader election for singleton loops | not started | **CORRECTNESS FLOOR SHIPPED (2026-06-25) — `replicas:2` safe; hardening + verification deferred** | `services/leader_election.py` (single session-scoped lock + `run_when_leader`), lifespan loop-gating, dispatcher CAS claim, Postgres keepalive tuning — all committed on `develop` (unpushed). Built on `docs/researches/orchestrator_leader_election.md`. Deferred: Task 5 (imap/delegation/digest/notify defense-in-depth) + Task 6 (two-replica failover test). |
