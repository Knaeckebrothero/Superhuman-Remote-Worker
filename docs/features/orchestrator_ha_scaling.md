...
> **Full-stack HA is NOT complete yet.** The remaining gap is the **data tier (Postgres)** — control/vector/keycloak/audit/litellm + Mongo + Neo4j all run `replicas: 1`, so an HA orchestrator in front of a single Postgres still goes dark if Postgres does. Interim posture: BYO managed/HA Postgres (`databases.*.externalHost`); batteries-included Postgres HA (CloudNativePG) is the open **M3** item. Optional polish also remains (L2 dispatch throughput, L3 cross-replica SSE, a hard-node-loss chaos test). See the reality-check matrix below.
**Filed:** 2026-05-12
...
| **M3** | Data-tier HA | Removes the **Postgres** SPOF → meaningful end-to-end HA. **NATS HA is already achievable via an external clustered hub (BYO — `nats.url`), so the only data-tier *building* left is Postgres.** | independent of M0-M2 (run in parallel); **Postgres HA required before the "thousand-agent cluster" claim is honest** | Postgres→CloudNativePG ~1-2 weeks; in-chart NATS clustering **deferred** (external = the HA path) | [[high_availability_setup]] P1-P2 |
