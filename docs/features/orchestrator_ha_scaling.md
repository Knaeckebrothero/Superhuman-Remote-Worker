...
**Status (2026-06-29): ORCHESTRATOR HA — ACHIEVED.** `replicas: 2` is the chart default and is correctness-safe + graceful-failover-safe. Shipped + verified: **M0** (active-passive hardening), **M1** (leader election — live-verified on dev under real traffic: graceful + hard failover, zero serving blackout, in-flight job survived three kills; two runtime bugs found + fixed — see `docs/tests/orchestrator_m1_leader_election_verification.md`), **M2-L4** (NATS handler replica-safety, live-verified on dev), and the **background-loop sweep** (all 28 boot loops audited; 2 gaps leader-gated; k3d two-replica verified — see `docs/tests/orchestrator_ha_background_loop_sweep.md`). Result: **no known double-fire path under `replicas: 2`**, and the **NATS coordination plane is already HA** (external 3-node hub; in-chart clustering deferred/BYO). The orchestrator is no longer a SPOF. _Last mechanical step: deploy the committed background-loop sweep fix to dev — it's k3d-verified but the last dev deploy predates it._

> **Full-stack HA is NOT complete yet — and as of 2026-07-30 the data tier is the ONLY thing left.** The rest of the application tier caught up with the orchestrator that day: Keycloak now runs production-mode with Infinispan/`jdbc-ping` clustering at `replicas: 2` (verified zero auth downtime through a pod kill), and Gitea's metadata moved to Postgres. So the remaining gap is exactly the **five single-replica Postgres instances** (`postgres`, `pgvector`, `keycloakdb`, `auditdb`, `giteadb` — Mongo and litellm are gone from the chart): an HA orchestrator *and* an HA Keycloak in front of a single Postgres still go dark when it does. Interim posture: BYO managed/HA Postgres (`databases.*.externalHost`, seams already built and exercised); batteries-included CloudNativePG is the open **M3** item. Optional polish also remains (L2 dispatch throughput, L3 cross-replica SSE, a hard-node-loss chaos test). Full decision framing — including that `postgres` + `keycloakdb` capture most of the benefit and `auditdb` is not worth HA — lives in [[high_availability_setup]] §P3.
**Filed:** 2026-05-12
**Refreshed:** 2026-07-30 — app tier completed around the orchestrator (Keycloak production-mode + `replicas: 2`; Gitea metadata on Postgres), leaving M3/data-tier as the sole remaining gap; see [[high_availability_setup]] §P3 for the path + scope decision.
**Refreshed:** 2026-06-24 — reconciled against current code (`main.py` grew ~15.5k → ~23.7k lines; deployment moved to Helm/Fleet; WS proxy removed; several coordination primitives shipped). All line references in this revision are current as of the refresh. **2026-06-26:** M0+M1 implemented, pushed, and k3d-verified (the matrix rows + Phase-0/1 checklists below are updated). **2026-06-29:** M2-L4 + the background-loop sweep shipped/verified; NATS clarified as external-HA (in-chart clustering deferred/BYO); **orchestrator HA declared ACHIEVED** — full-stack HA still pending Postgres (M3).
...
|---|---|---|---|
| **Track 1** active-passive hardening (probes / `preStop` / grace / PDB) | not started | **SHIPPED (chart, on `origin/develop` 2026-06-26); live chaos test operator-pending** | `preStop` drain + `terminationGracePeriodSeconds` + `startupProbe` + tuned probes + orchestrator PDB landed in `helm/` (M0). Behavioral chaos test is operator-run on dev — see `docs/operations/orchestrator_failover.md`. |
...
| **Data tier** prerequisite (Postgres HA; NATS) | out of scope here | **NATS: HA via external hub (BYO) — in-chart clustering DEFERRED. Postgres: NOT STARTED** | The chart's *bundled* NATS (`internal: true`) is single-replica; pointing `nats.url` at an external clustered hub gives HA (dev/prod-private run a shared 3-node hub). Postgres still single-replica (no CloudNativePG). See [[high_availability_setup]]. |
...
M3 Data-tier HA ──────────────┴──> meaningful END-TO-END HA
   (Postgres HA; NATS=external)      + active-active at scale
   [umbrella doc]
```
...
| **M3** | Data-tier HA | Removes the **Postgres** SPOF → meaningful end-to-end HA. **NATS HA is already achievable via an external clustered hub (BYO — `nats.url`), so the only data-tier *building* left is Postgres.** As of 2026-07-30 it is also the only *anything* left: Keycloak and Gitea finished their app-tier work that day, so every remaining availability gap is a database. | independent of M0-M2 (run in parallel); **Postgres HA required before the "thousand-agent cluster" claim is honest** | BYO managed Postgres = zero chart work; in-chart CloudNativePG ~1-2 weeks; in-chart NATS clustering **deferred** (external = the HA path) | [[high_availability_setup]] §P3 |
...
- **Minimum honest "HA orchestrator" claim = M0 + M1, with HA NATS + HA Postgres as documented bring-your-own postures.** A single replica failure is tolerated and `replicas: 2` is safe (M1 + M2-L4 + the loop sweep → no known double-fire). The coordination plane isn't a SPOF *provided NATS runs HA* — and the chart already supports that via `nats.url` pointing at an external clustered hub (the official nats chart does 3-node + JetStream out of the box; this is exactly how dev/prod-private run today). So **in-chart NATS clustering is DEFERRED** — same rationale as Postgres (`databases.*.externalHost` for external/managed/HA Postgres): for both, BYO external HA is a legitimate OSS stance and the cheapest path to a true claim. Bundling clustered NATS/Postgres into the chart is the *Full* claim below, not the minimum.
...
- ~~IMAP double-poll / dispatch double-assign if anyone sets `replicas: 2`~~ — **fixed by M1 (2026-06-25):** the loops are leader-gated and the dispatch/imap/delegation/digest/notify paths are DB-guarded. The default is now `replicas: 2` (M1 live-verified on dev + flipped 2026-06-26); the foot-gun is gone.
...
- **NATS HA — DEFERRED (BYO external hub = the HA path).** Only the chart's *bundled* hub (`nats.internal: true`, `helm/templates/nats/`, clustering disabled) is single-replica. For HA, point `nats.url` at an external clustered hub — the official nats chart does 3-node + JetStream out of the box, and **dev/prod-private already run exactly this** (one shared 3-node homelab hub + per-install leaf nodes on a shared VM cluster, scoped by `orchestratorId`; see [[orchestrator_ha_state]]). Making the *in-chart* hub clusterable is deferred — low priority while external HA is a one-`nats.url`-line option. [[high_availability_setup]] Priority 2.
...
### Phase 1 — Track 2 Layer 1: Leader election — CODE SHIPPED 2026-06-25 (plan: `docs/superpowers/plans/2026-06-25-orchestrator-m1-leader-election.md`; verification: `docs/tests/orchestrator_m1_leader_election_verification.md`)
- [x] `orchestrator/services/leader_election.py` — a **single** leadership lock (`run_as_leader`) via **session-scoped** `pg_try_advisory_lock(LEADER_ID)` on a **dedicated, reconnecting** pooled connection, with random startup jitter + graceful step-down (release in `finally`). _(As-built: also added `run_when_leader(make_coro, shutdown)` — a wrapper applied at the lifespan registration site that runs/cancels a loop as leadership flips, instead of editing a per-tick gate into each loop body.)_
- [x] Lock-ID registry module: `database/lock_ids.py` — `LEADER_ID = 0x5352575F4C454144` ("SRW_LEAD") alongside `LOCK_ID`/`MAINT_LOCK_ID` (packed-ASCII int64; never collide).
- [x] The elected leader starts the singleton loops registered in `lifespan` (9 loops wrapped with `run_when_leader`); the already-HA-safe loops + idempotent sweepers + `main_cloud_listen_task` run on all replicas. `_trigger_dispatch` also gated on `is_leader`.
- [x] **Dispatcher correctness — CAS landed (the floor); `SKIP LOCKED` deferred to M2.** `PostgresDB.claim_job_for_agent` (CAS `WHERE assigned_agent_id IS NULL AND status IN ('created','paused')`) claimed at the dispatcher Phase-1 call site **before** notifying the agent — closes the dual-leader double-assign. `FOR UPDATE SKIP LOCKED` on the candidate scan (`get_dispatchable_jobs`, `postgres.py:2569`) is a throughput optimization, not a safety requirement once the CAS is in place → stays M2.
- [x] **Failover tuning:** `tcp_keepalives_idle/interval/count = 10/10/3` on the Postgres deployment (`helm/templates/databases/postgres.yaml`) → ~40s hard-failure detection. _(`idle_session_timeout` intentionally **not** set — it's global and would reap idle pool connections; the leader's 10s `SELECT 1` keep-warm holds its own session.)_
- [x] **External-pooler guard:** loud warning in `helm/values.yaml` + `helm/values.example.yaml` (session locks break behind a txn-mode pooler). _(Doc/values warning, not a runtime self-check — SRW is verified pooler-free; the warning covers the external-Postgres path.)_
- [x] Harden other non-idempotent loops at the resource (M1 Task 5, defense-in-depth for the ~40s dual-leader window): **imap** insert-as-claim via a dedicated `processed_inbound_emails` table (migration 0037) — chosen over a `UNIQUE(email_message_id)` retrofit on the hot `message_log` to avoid touching that write path / dedup-of-existing-rows; **delegation resume** CAS (`claim_delegation_resume`, waiting→paused); **digest** atomic claim (`claim_pending_notifications`, delivered_at NULL→NOW RETURNING, release-on-failure); **notify** claim-before-send (partial unique index `uq_tn_sent_request_kind`, migration 0038, `ON CONFLICT DO NOTHING`).
- [x] Log "leader acquired/lost" at INFO.
- [x] Tests (pytest + `PostgresContainer`): leader exclusivity + failover, `run_as_leader`/`run_when_leader`, atomic job claim, + the four Task-5 resource guards — **16 tests green** (`tests/test_leader_election.py`, `test_job_claim.py`, `test_imap_dedup.py`, `test_delegation_resume_claim.py`, `test_digest_claim.py`, `test_notify_dedup.py`).
- [x] **Unlock:** `replicas: 2` is **correctness-safe** (the singleton loops are leader-gated and every correctness-critical path is DB-guarded). **Live two-replica failover verified on dev 2026-06-26** (graceful + hard, zero serving blackout, in-flight job survived three kills; two runtime bugs found + fixed — see the verification doc). Remaining: the deliberate chart-default replica-count flip (`helm/values.yaml`, Phase 5 / M4) after the dev soak.
...
- [x] Bump `orchestrator.replicas: 2` + `pdb.minAvailable: 1` in `helm/values.yaml` — **done 2026-06-26** (M1 declare-done; live-verified on dev first). The Fleet `values-experimental.yaml` dev overlay already carried 2; this flips the chart default for fresh installs + the next release.
...
- **2026-06-29 (NATS HA posture):** in-chart NATS clustering **DEFERRED** — HA NATS is a documented BYO-external posture (`nats.url` → external clustered hub), the same stance as BYO Postgres. dev/prod-private already run a shared external 3-node JetStream hub (per-install `nats-leaf` + `vm-controller` on a shared VM cluster, scoped by `orchestratorId`); the chart's bundled `internal: true` hub (single-replica) is **unused** in those environments. Corrects the earlier "NATS is a flat SPOF" framing — that described only the dormant in-chart template.
