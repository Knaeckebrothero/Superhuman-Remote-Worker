...
- [ ] **2. No-op compounding guard (the deferred fast-follow from [[loop_repo_compounding]]).** In `_spawn_loop_job` (`orchestrator/main.py:9992`) capture the project repo's `main` HEAD for execution roles; in `_advance_project_loop` (`main.py:10037`) compare and record `jobs.merge_status` (`advanced` / `no-op`). A completed execution job with `no-op` logs loudly and shows in the Loop tab; optional strict mode (per-loop flag) counts it as a failure toward the cap. Evidence: F29's 41 M-token silent loss — and the same guard also catches future provisioning regressions on *any* backend. One Gitea API call per execution iteration.
  **→ Elevated by F40 (07-02 deep dive): this guard is the loop's ONLY viable artifact-integrity check.** No agent role can own it — critics run in fresh workspaces with no reachable `main` (the cycle-3 critic actively hunted for the code, found nothing, and rationalized it as workspace isolation), and destroyed developers' "SHIPPED GREEN" retros stay `active` in the KB, so the evidence chain is self-referentially KB-validating-KB. Artifact truth must come from the orchestrator comparing `main` HEADs; consider also invalidating `state`/retro notes when the guard detects a no-op.
