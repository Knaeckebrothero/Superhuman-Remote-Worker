...
> **RESOLUTION (2026-06-28) — Gap A + Gap B both FIXED, deployed + LIVE on dev.** Implemented via the roadmap `docs/features/route_all_models_through_litellm_gateway.md` (commit hashes post-rebase). **Gap A (bypass → zero usage rows): RESOLVED** — P1 (`3e7b67bf`) registers system + codex models in the gateway; P2 (`3bb72235`) routes them through it behind a per-provider canary (`LITELLM_GATEWAY_ROUTED_PROVIDERS`; a `"*"` wildcard routes all). **Gap B (null `cost_usd`): RESOLVED** — P3 (`27d026ac`) makes `materialize_llm_usage` consume LiteLLM's authoritative per-request `spend` as a dedicated `unit='request'` cost dimension (token rows stay cost-free; `SUM(cost_usd) == spend`, no double-count). `usage_rates` is untouched and now prices `category='compute'` only. P0 (`a5bae034`) added the gateway-down→direct fallback. **Production proof (dev `sha-1163fd8`):** codex GPT-5.5 **$100.79** + gpt-5.4-mini **$15.37** now costed in `usage_events` (was $0); the previously-bypassed paid lane is metered.
>
> **Current-state caveat (the bug isn't 100% closed on dev):** the canary is committed at `openrouter` only (`1163fd8d`), so **gemini (system/google) STILL bypasses** the gateway on dev — enabled, paid, and unmetered. Endpoints (gemma/codex-GPT) and minimax (openrouter) are covered; gemini is the last bypassing paid lane. Closing it is one flip: add `google` to the canary, or `routedProviders: ["*"]` (gated by the P5/HA SPOF decision — see roadmap §7).
