...
**Status:** Root cause **confirmed by code trace** (not yet reproduced with the fix applied) · fix **designed + direction approved** (keep the PWA service worker; bypass the non-cacheable request class — "approach (a)") · **not yet implemented, not yet verified live**
