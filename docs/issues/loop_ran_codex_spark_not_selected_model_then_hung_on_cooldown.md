...
**Status:** Investigation complete, root causes isolated on the live main cluster. Three distinct defects, causally chained (A → B → C below). **Track 1 (P1 + P2) implemented + verified (local + k3d). Track 2 (Defect C): C1 (cooldown-aware fail-fast) + C2 (consecutive-failure circuit breaker) implemented + unit/integration-verified; C3 deferred. All 2026-06-27, uncommitted on `develop`** — see the Implementation status notes under Sequencing / Track 2.
