...
1. **Close the race (primary).** In `resume_thread`, either `await
   _late_cloud_setup(...)` before scheduling `_reprovision`, or have `_reprovision`
   await the late-setup task/registry entry before `_send_session_attach` — the
   same "concurrent attach can await this task" pattern
   `_schedule_protected_engage` already uses at `:25248`.
2. **Make the agent recoverable (defence in depth).** A session that attached
   degraded should re-resolve cloud config once at the next turn boundary rather
   than staying `workspace_sync = None` for life. Turns a permanent failure into a
   few-seconds-late start.
3. **Bootstrap the cloud account on resume**, mirroring create: use
   `backend.ensure_user(...)` instead of a lookup-only resolve, so the share can
   succeed on threads whose owner has never signed into the cloud.
