...
**Status:** investigated 2026-07-07 from two failed Better-Resavio loop iterations on the main cluster. Root cause confirmed by elimination against the DB state + code paths. **Steps 1-3 IMPLEMENTED + unit-verified on `develop` (uncommitted): orchestrator guard (`completion.py`), agent-side error clear (`graph.py`), +7 tests (255 passing across affected suites); ruff clean. Step 4 endpoint drill VERIFIED live on k3d 2026-07-07** (the exact incident payload `{version_upgrade freeze + coincident error}` → `paused` + freeze stashed to `context.last_freeze_data` + `freeze_data` column NULL; control `{bare error}` → `failed`; guard `WARNING` logged). **Remaining: Step 4 full-agent drain drill (optional; graph.py side is unit-covered) + Step 5 commit/rollout/post-deploy watch.** The exact `error_message` string on the two jobs was not retrieved (see "Open questions"). Symbols/line numbers current as of this date.
