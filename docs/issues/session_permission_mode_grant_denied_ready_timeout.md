...
**Status:** **ROOT CAUSE CONFIRMED + empirically verified on dev** · quick-fix applied (granted the affected user `permission_mode: auto_accept`) · **ALL phases 1–5 IMPLEMENTED + k3d-verified** — Phases 1–4 **committed** on `develop` (`5b05b6eb`…`66809484`), Phase 5 (default ceiling → `auto_accept`) uncommitted (1 provisioning pre-flight · 2 UI greying · 3 fail-loud-at-write-paths · 4 agent-side disambiguation · 5 default ceiling raised to `auto_accept`) — layered design + roadmap below · **regression** since `a7ad2be0` (2026-06-18). Affects any **non-admin** who picks `auto_accept`/`autonomous` for a session; admins are unaffected (grant bypass), which masked it.
