...
**Status:** **ROOT CAUSE CONFIRMED + empirically verified on dev** · quick-fix applied (granted the affected user `permission_mode: auto_accept`) · **Phases 1 (provisioning pre-flight) + 2 (UI greying) + 3 (fail-loud-at-write-paths) IMPLEMENTED + k3d-verified, uncommitted on `develop`**; Phases 4–5 pending — layered design + roadmap below · **regression** since `a7ad2be0` (2026-06-18). Affects any **non-admin** who picks `auto_accept`/`autonomous` for a session; admins are unaffected (grant bypass), which masked it.
