...
**Status:** **ROOT CAUSE CONFIRMED + empirically verified on dev** · quick-fix applied (granted the affected user `permission_mode: auto_accept`) · code fixes NOT yet implemented — layered design + 5-phase roadmap below · **regression** since `a7ad2be0` (2026-06-18). Affects any **non-admin** who picks `auto_accept`/`autonomous` for a session; admins are unaffected (grant bypass), which masked it.
...
**Phase 1 — provisioning pre-flight (Layer 3).** ★ highest value — kills the 5:40 wait + the wasted pod.
- *Build:* factor `_session_grant_violations(thread, metadata) -> list[str]` around `_enforce_dispatch_grants`; call at the top of `provision_or_assign` (`services/provision_or_assign.py`) and `_do_prepare` (`routers/sessions.py`). On violations → `lifecycle_emit(... "failed", reason=...)` and return *before* pool-attach / `provision_agent`.
- *Acceptance:* denied-mode session emits `session.lifecycle: failed{reason}` in ~1 s; **no** `srw-agent-s-*` pod created; cockpit card shows the violation.
- *Tests:* unit on `provision_or_assign` (GrantDenied → failed-emit, `provision_agent` **not** called) + `_do_prepare`.
- *Verify (k3d):* non-admin no-grant session → fast fail with reason; `kubectl get pods` shows no session pod spawned.
