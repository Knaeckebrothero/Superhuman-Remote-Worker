...
**Status:** **Largely fixed.** Write-side fix (A — strip config from per-row audit metadata) **committed `7ea0d798` + deployed to dev**; read-side lean projection (B) shipped as Phase 1 of the debug-view refactor (see `docs/features/debug_audit_view_refactor.md` §0), and the Cockpit debug view no longer touches the bulk path at all. **Two gaps remain:** (1) the existing-rows **backfill (#4) was run on local k3d only — NOT dev/prod**, so the *old* `/audit/bulk` endpoint can still OOM on `19707fa1`; (2) the bulk endpoints + `get_*_bulk` stay because **MCP `get_audit_bulk` still uses them** (migrate it to the lean/paged path, then remove). Root cause isolated + **measured on the live main cluster** (below). Originally two distinct defects (write-side bloat + read-side amplifier); either fix alone stops the OOM.
**Found:** 2026-06-27. Job `19707fa1-1788-4eda-a296-8b108429b108` ("Loop iter 3 · DEVELOPER — Build an ERP system for Hotel Rheinland in Bad Orb"), project `68137e29-6b1f-4f1b-a0c1-4e6dc2be3f9a`, owner `knaeckebrothero` / `overlygenericaddress@pm.me`, **main cluster** (ns `superhuman-remote-worker`, orchestrator image `sha-f0b1188`).
...
**1. Primary (A) — stop persisting heavy job-level blobs into per-row audit metadata.** Separate "config the agent boots from" from "audit metadata". Strip a denylist (`resolved_config`, `config_override`, `datasources`, `repositories`) from whatever job-level `metadata` the agent records per audit event — store them **once per job** (they already live in `jobs.resolved_config` etc.). This alone drops the job's audit from ~480 MB to ~2.5 MB.

**2. Defense-in-depth (B) — don't ship `metadata` from the bulk/list path.** The Cockpit audit timeline does not need 127 kB of config per row. Drop `f.metadata` from `_STITCH_CORE` (or add a lean projection for `get_job_audit_bulk` / `get_graph_deltas_bulk`), and/or stream rather than `fetch()`-all, and/or lower the per-page limit well below 5,000.

**3. Capacity (mitigation, not a fix).** `requests.memory=256Mi` / `limits.memory=2Gi` is low for an ~11.5k-line FastAPI app that serves bulk endpoints. Raise it to widen the margin — but without #1/#2 a large-enough job still OOMs, so this is not sufficient alone.

**4. Cleanup / backfill.** Existing rows already carry the duplicated config (partitions are GB-scale). Either a one-time `UPDATE agent_audit SET metadata = metadata - 'resolved_config' - 'config_override' - 'datasources' - 'repositories'` (per live partition), or let monthly partition rotation age it out. Separately, prune this thread's 447 MB checkpoint blobs (cancelling the job fires `delete_checkpoint_thread`).
