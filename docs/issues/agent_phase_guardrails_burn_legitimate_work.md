...
**Status:** as of 2026-07-15 all findings are **open / unbuilt** except as noted below.

**2026-07-30 update (Centurion-postmortem P0/P1 waves,
`docs/issues/officer_blind_reads_and_worker_bureaucracy.md`):** several adjacent
fixes landed that change this doc's backdrop — #13 fixed (Gitea-backed reads);
the *feedback-resume* path now archives in-flight todos instead of silently
wiping them, and `archive_with_failure_note` gained `phase_label`/`heading`
params (P1-A `dc33a86b`), which makes #3's preserve-pending variant a small
change now; `min_todos` is wired and lowered to 2 and the strategic transition
template is 2 todos (P1-B `9170558c`), shrinking the re-plan cost every rewind
in #3/#6 forces; jobs can declare `required_deliverables` validated at the seal
(P1-C `6cd60bb2`), which gives #11 a contract to fail fast against (the
stub-brief check itself is still unbuilt); and cancel/pause now push an
evidence snapshot (P1-D `dee67f2a`), so guardrail misfires that end a job no
longer destroy the work they interrupted. #1, #2, #4, #6, #7, #8, #10, #12
remain untouched.
...
| 11 | **No brief fail-fast.** `task_brief.md` was a **119-byte stub** echoing the job description; `instructions.md` was generic default boilerplate. Nothing checks that a real brief arrived. The agent self-generated a 27.8 KB `plan.md` with 13 deliverables on top of one sentence and worked 9.4h against it. | job workspace; dispatch path | Open — partial substrate landed 07-30: jobs can declare `required_deliverables` at dispatch, rendered into `task_brief.md` and gate-validated at the seal (P1-C `6cd60bb2`); a job with a declared contract can no longer invent its own deliverable set, but nothing yet *refuses* dispatch on a stub brief |
...
| 13 | **Live healthy job reads as dead through the API.** `get_todos`, `get_workspace_overview`, `list_todo_archives`, and `get_job_log` all return empty/not-found for this job while `list_job_files` and `list_job_commits` work fine. `get_job_progress` reports **0.0%** after 18 completed phases. Through the Cockpit this job looks idle or dead; it is neither. | MCP/orchestrator read path | **Fixed 2026-07-30** `70ca9461`: `/todos*` + MCP workspace tools now Gitea-backed (P0-B of `officer_blind_reads_and_worker_bureaucracy.md`) |
...
- **Strategic-phase overhead is ~50% of wall clock.** Every strategic phase re-reads and rewrites a
  ...
  rewind (#3-#6) costs one of these on top of the lost todos. *Now quantified and attacked:*
  `officer_blind_reads_and_worker_bureaucracy.md` annex C is the ledger; P1-B (07-30) merged
  REVIEW+ADAPT, halved the template to 2 todos, killed the forced guide re-reads, and deleted the
  audit protocol (revert `4eba5d47` to restore).
