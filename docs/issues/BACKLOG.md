...
| [cloned_repo_checkout_cannot_reach_non_default_refs](cloned_repo_checkout_cannot_reach_non_default_refs.md) | **Live blocker on Better Resavio.** `git_show`/`git_log`/`git_diff` are bound to the job's own repo and silently answer `fatal: bad object` for an attached repository, and attached repos have no read tool at all (5 repo_* tools, all writes). Cost a worker run + an operator page on a workspace that was healthy. |
