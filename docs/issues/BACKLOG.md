...
| [ci_deploy_race_reverts_image_tags](ci_deploy_race_reverts_image_tags.md) | NEW 08-13: overlapping CI runs write **stale per-component image tags**, silently reverting a just-verified dev deploy to older code (observed: `sha-0ef826c` → `sha-9dd5175`, both runs green). Breaks the *deploy → verify → report* loop — a fix can be verified truthfully and be gone minutes later. Compounding: image tags are **mutable** (a dispatch rebuilt `sha-9dd5175` with different content), so the running tag does not identify the running code and rollback-by-tag is untrustworthy. Diagnosis + proposed guard (ancestry check against `provenance.*.sourceRevision`, needs `fetch-depth: 0` on the deploy checkout) in the doc; **not implemented** — a broken `deploy-experimental` breaks every dev deploy. |
