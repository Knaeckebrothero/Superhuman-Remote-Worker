...
| [cloned_repo_checkout_cannot_reach_non_default_refs](cloned_repo_checkout_cannot_reach_non_default_refs.md) | **Live blocker on Better Resavio.** A repo datasource checkout carries only `origin/main`; `repo_pull` fast-forwards the checked-out branch only and no repair tool is bound. Branch/commit work is impossible. |
