...
| doc | why now |
|---|---|
| [officer_control_plane_post_implementation_audit](officer_control_plane_post_implementation_audit.md) | **INDEX — keep `auto_pull` off.** BP-07, BP-08/BP-13, and BP-10 passed their deployed main-dev gates on 2026-08-17. The remaining supported-control, authority, evidence, and unattended release/live gates still have to pass. |
...
| [deliverable_contract_satisfied_by_a_note_about_failure](deliverable_contract_satisfied_by_a_note_about_failure.md) | **Unattended delivery is uncertifiable.** Three consecutive Better Resavio jobs sealed `completed` with the gate passing, having shipped nothing — a `kb:` slug is satisfied by a note describing the failure. Blocks trusting any loop or officer run. |
...
| doc | why |
|---|---|
| [category_expert_default_skipped_on_direct_dispatch](category_expert_default_skipped_on_direct_dispatch.md) | **The builder assignment exists and never runs.** `CATEGORY_DEFAULT_EXPERT[executor] = developer`, but `classify_ticket`/`resolve_expert` are reached only inside `if job.ticket:` — so hand-dispatch (the only enabled mode) falls through to `general-worker` with no shell. 8/8 Better Resavio jobs. Do NOT fix by adding an expert key to officer.slots. |
...
| [pat_action_scopes_are_never_enforced](pat_action_scopes_are_never_enforced.md) | **Security, latent.** PAT action scopes are validated, stored and loaded, but no route reads them — the "action-scope decorator" the code defers to does not exist, so a `jobs:read` token has full write + admin reach. Zero PATs issued today, so blast radius is currently nil; arms itself on the first scoped token, and multi-tenancy / the public FSL release both assume this control works. |
...
| [ci_deploy_writes_stale_image_tag](ci_deploy_writes_stale_image_tag.md) | NEW 08-13: when CI runs overlap, the deploy job writes a component identity computed at run *start*, so it can land a tag already behind the branch (observed: `sha-0ef826c` written 3½ min after a newer orchestrator commit hit origin). **Self-correcting** — the next component build fixes it. Guard implemented (ancestry check + `fetch-depth: 0`) but **not yet exercised in CI**; needs two overlapping runs. **Note:** this doc's first version claimed a *revert* and *mutable tags* — both retracted, see §Retraction; the ordering was never checked with `merge-base --is-ancestor`. Remaining real defect: the deploy commit's message reports the run sha while values-experimental.yaml reports component identities. |
...
| [cloned_repo_checkout_cannot_reach_non_default_refs](cloned_repo_checkout_cannot_reach_non_default_refs.md) | **FIXED on develop 2026-08-16, undeployed.** git_* now takes `repo=` and reads attached repositories, an unresolved ref names the repos that might hold it, and the git group is suppressed when the agent has shell. Cost a worker run + an operator page on a workspace that was healthy. |
| [queued_digest_suppresses_officer_escalation](queued_digest_suppresses_officer_escalation.md) | **Unattended runs cannot reach you.** The officer treats a queued-but-undelivered digest as "reported" and stops escalating; ~14 wake cycles and ~$23/day of silence against a blocker only a human could clear. Consequence-half of F4. |
| [public_repo_ships_a_workstation_ip_and_operator_emails](public_repo_ships_a_workstation_ip_and_operator_emails.md) | **Security, public repo.** A workstation IPv4 is committed at HEAD and in history, against the standing public-IP policy. Credentials verified clean. |
...
| [officer_commission_can_silently_de_arm_its_workers](officer_commission_can_silently_de_arm_its_workers.md) | An officer can commission a worker with zero connectors — and therefore no repo tools — two independent ways. Self-healed once, so it is intermittent and unreproducible on demand. |
| [officer_per_job_model_choice_is_silently_discarded](officer_per_job_model_choice_is_silently_discarded.md) | Roster slot pinning replaces the officer's per-job model and backend with no notice, so the officer reasons about a lever that does nothing. |
