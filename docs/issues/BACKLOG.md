...
| doc | why |
|---|---|
| [kb_connector_token_auth_over_http_fails_invisibly](kb_connector_token_auth_over_http_fails_invisibly.md) | Token auth is (correctly) refused over http, but the ValueError is raised during source construction — before anything writes kb_index_watermark. The connector retries and fails every tick while the cockpit shows a stale, unrelated error. Observed 08-13: over an hour of silent 15-min failures behind a misleading badge. |
| [kb_sweep_indexes_archived_projects_and_starves_connectors](kb_sweep_indexes_archived_projects_and_starves_connectors.md) | Archived projects are swept forever (no status filter), and because natives sweep before externals in the same tick, a native KB that can't finish blocks every external KB connector from indexing. Observed live 08-13: an archived 3,137-note project stuck `indexing` for 37+ min with `last_success_at` a month stale, starving a new connector. Per-project workaround applied; no code fix. |
