...
| doc | why now |
|---|---|
| [officer_control_plane_post_implementation_audit](officer_control_plane_post_implementation_audit.md) | **INDEX — keep `auto_pull` off.** BP-07 and BP-10 passed their deployed main-dev gate on 2026-08-17. BP-13 is local/k3d green, but its BP-08 main-dev retry slice and the remaining release/live gates still have to pass. |
| [knowledge_metadata_retry_commits_then_projection_fails](knowledge_metadata_retry_commits_then_projection_fails.md) | **P0 auto-pull release gate (BP-13/BP-08 residue), implemented and k3d-gated locally.** Typed canonical snapshots, sweeper-won reread, exact disposition projection, and direct-reindex settlement have real-pgvector plus real HTTP/Gitea evidence; commit/deploy the repair and rerun the bounded BP-08 slice on main dev. |
