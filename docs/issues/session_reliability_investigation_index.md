...
| 3 | Service-worker `/api/**` freshness-cache audit — SSE streams are protected only by a *fragile per-URL* `?ngsw-bypass=true`; the stateful `/connection` handshake, binary downloads, and IDE proxy are still cached. **⚠️ The prescribed `dataGroups.urls` `!`-negation carve-out does NOT work** — ngsw supports negation only in `assetGroups.files`/`navigationUrls`, and (more fundamentally) the SW re-issues every request via `scope.fetch` regardless of group config; `ngsw-bypass` is the only real opt-out. See correction + confirmed file-upload manifestation. | same doc — "Update 2026-06-27" follow-ups 1 & 2 + live-endpoint inventory · **corrected + resolved by** `cockpit_service_worker_breaks_file_uploads.md` (now in `docs/done/`) | ✅ **Resolved** (`1195b54d`, deployed + live-verified 2026-07-15): `ngsw-bypass` on all non-safe methods via `auth.interceptor.ts` + `api-cache` `/api/**` dataGroup **deleted** — no API endpoint is SW-cached anymore, which also retires the stale-`/connection`/binary-download concerns. SSE keeps its per-URL bypass (harmless belt-and-suspenders). |
