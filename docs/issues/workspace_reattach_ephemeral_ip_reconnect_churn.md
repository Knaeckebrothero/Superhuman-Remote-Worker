...
**Status:** Filed + diagnosed live on k3d (2026-06-29). **Not fixed.** Discovered by the Phase-2 crash-recovery E2E for PVC-backed workspaces (`docs/features/workspace_pvc_branch_a_implementation.md` §Phase 2). The PVC-reattach *data* path works; the *agent reconnect* does not, so recovery fail-louds instead of resuming.
