...
**Progress (2026-07-01):** P0 **done** (litellm disabled on homelab; routing direct). **P1 (restore monitoring in-process) — Slices 1–3 DONE + k3d E2E-verified** (32 tests pass, `ruff` clean; **uncommitted `develop`**). In-process metering (audit `llm_requests` → priced `usage_events`) is live on k3d; the live run caught + fixed a critical id-vs-timestamp cursor bug. Remaining: commit, then homelab cut + P2/P3 cleanup. See § Plan / § k3d E2E.
