...
**Progress (2026-07-08):** P0 **done** (litellm disabled on homelab; routing direct). **P1 (restore monitoring in-process) — Slices 1–3 DONE + k3d E2E-verified** (32 tests pass, `ruff` clean; **uncommitted `develop`**). In-process metering (audit `llm_requests` → priced `usage_events`) is live on k3d; the live run caught + fixed a critical id-vs-timestamp cursor bug. **Deployment cleanup is now done:** the Helm chart and local/experimental values no longer render LiteLLM or its dedicated DB, and the orchestrator no longer starts the LiteLLM sync/quota loops. Remaining: deeper routing-code deletion after direct-routing soak. See § Plan / § k3d E2E.
...
- **P1 — Restore monitoring in-process** (the one real gap). **Design (decided):** the materializer emits **cost-free** `prompt-token` / `completion-token` `usage_events` rows keyed by model id; `UsageLedger.record_events` (`usage_ledger.py:179-184`) already auto-prices any token row via `UsageRates.resolve(category, resource, unit, ts)` against the effective-dated `usage_rates` table (`migrations/app/0033`). So "cost" = **seed `usage_rates`** — no per-request cost row, no gateway `spend`. Source = the auditdb **`llm_requests`** SQL table (`audit_writer.py:102`), written by **both** job agents and sessions (`persistent_app.py:3304`), same Postgres as `usage_events` — no Mongo. Free wins vs the gateway: **per-job attribution** (`job_id`) + **session coverage** + backfillable.
  ...
  - **Slice 3 — re-point + pricing wiring. ✅ DONE + k3d E2E-verified (uncommitted `develop`).** `llm_usage_poll_loop` (`main.py:1553`) now calls `materialize_llm_usage_from_audit(audit_db.pool, postgres_db.pool, usage_ledger)` — gated on the two pools + ledger (LITELLM gate dropped). **Forward-only anchor = `SELECT max(timestamp)`** at startup (not max-id; see cursor note), so re-pointing never re-meters history under `source='audit'`. New `pricing_sync_task` runs `llm_pricing_sync_loop` (openrouter_pricing.py: enumerate ALL catalog rows via `postgres_db.list_models` → `(model_id, params_json.pricing_id)` pairs → `sync_llm_rates`, 6 h + on startup). `litellm_sync_task` **kept** (gateway still routes traffic on k3d/prod-not-yet-cut; only the usage *materialization* switched, so no double-count — the litellm-spend poll is simply gone). `materialize_llm_usage` import dropped.
...
- **k3d E2E verification — ✅ DONE (2026-07-01).** Deployed Slices 1–3 to k3d (Tilt/uvicorn reload) and drove the full path against live DBs.
  ...
  - **Residual / follow-ups:** forward-only anchor skips the homelab disable-gap (backfill = a one-off lower anchor); no `pricing_id` UI yet (DB-only); commit + homelab cut still pending; `litellm_sync_loop` + chart templates removal is the later P2/P3 cleanup.
...
- **P3 — Cleanup.** **Deployment cleanup done 2026-07-08:** deleted `helm/templates/litellm/*`, `helm/templates/databases/postgres-litellm.yaml`, `databases.litellm`, the chart/overlay `litellm:` values, `LITELLM_*` chart config/env, and the orchestrator LiteLLM sync/quota startup tasks. Remaining code cleanup: remove the stale `main.py:1232` docstring (describes per-job keys that never shipped) and simplify dispatch routing to drop the gateway branch after direct-routing soak.
