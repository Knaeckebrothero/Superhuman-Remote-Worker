...
> auxiliary-LLM service, feedback injected into the agent loop, and cited cloud
> documents anchored by a saved snapshot. Phases 1–3 shipped (DB collapse;
> verifier→aux + D4 feedback; cloud snapshot-anchor + original-bytes blob +
> on-view drift/view-original endpoints); see [[citation_engine_integration]] for
> the current architecture + status. The *feature* goals here (shared source
> library, annotations/tags, hybrid search) still stand and are built.
