...
> One address book for every person an agent may reach — email today, WhatsApp next — owned by a user, linked to projects, visible in Cockpit, and readable by agents through a virtual `contacts/` directory.

**Status: SHIPPED — deployed and live-gated on dev 2026-08-01.** Designed 2026-07-26→28 (brainstorm; Cockpit layout picked via visual mockups), spec written 2026-07-29, §Agent surface amended 2026-07-30 onto [[virtual_directories]]. Every slice is now implemented and running on dev.

| Slice | State |
|---|---|
| Schema + backfill | `0076_contacts_normalize.sql`, applied `success=true`, 0 dirty rows |
| DB layer, router, `send_agent_message` rewire, legacy retirement | Shipped, reviewed, live-gated |
| Cockpit `/contacts` | Shipped; rebuilt on the house UI kit 2026-08-01 |
| Agent surface (`contacts/` virtual dir) | Shipped with [[virtual_directories]] Slice 1, live-gated (agent `sha-f41970a`) |

**Agent surface.** An agent on a project reads `contacts/README.md` and `contacts/<slug>.md` through its ordinary file tools, served live from the DB by `ContactsProvider` (`src/core/virtual_dirs/contacts_provider.py`) via `GET /api/contacts/internal/list`, which derives the project **server-side** from job/thread identity — an agent cannot ask for another project's contacts. Nothing is written to the workspace filesystem. The provider renders through `src/core/contact_files.py` (`contact_slug` + `render_contact_md`, imported at `contacts_provider.py:14`), so that module is live, not dormant; the provider adds the `README.md` index on top.

**Owed.** Two things, neither blocking use:
1. **Phase-2 migration** — re-run the backfill sweep, then drop `external_contacts`. Deferred by design to a later release. **Read §Data model's phase-2 caution before writing it**; a naive re-run overwrites names users have edited in Cockpit.
2. **Agent-path verification gaps** — a project *with* contacts populating `contacts/<slug>.md`, and the mid-session freshness window, are still unverified. Tracked in `tests/virtual_directories_test_coverage.md` §2.1.

**Live gate, 2026-08-01** (k3d is unusable on the dev machine, so dev is the gate). Against the deployed image: all four `resolve_contact` branches, both opt-in defaults, duplicate→`None`, auto-primary, promotion+demotion, opt-in reset on address edit, all three `list_contacts_for_user` filters, link/unlink, cascade delete. In the browser: create with two channels, address normalization (`UI-Gate@Example.ORG`→lowercased, `+49 170 555-(0)77`→`+49170555077`), chip annotation `whatsapp·opt-in pending`, read-only expansion, channel select disabled on saved rows, delete dialog naming the affected project, and — after the fix below — a successful edit.

**The bug the gate caught.** `update_contact` raised `AmbiguousParameterError` at PREPARE time on *every* call: `$3` appeared only in `IS NULL` / `NULLIF($3, '')`, neither of which pins a type, and asyncpg prepares untyped. **Every `PATCH /api/contacts/{id}` returned 500**, so editing a contact was impossible. Fixed with `$3::text` plus the integration test that was missing — `update_contact` was one of six DB methods two reviews flagged as untested, and it was the one that broke. Nothing in the suite could have caught it; only a real database could. Diagnosis trap: the browser reports that 500 as a **CORS error** (`No 'Access-Control-Allow-Origin' header`), because unhandled-500 responses bypass the CORS middleware — from DevTools alone it looks like misconfigured CORS.
...
*(Historical, 2026-07-30→08-01: a boot-time materialization version shipped first — `_gather_project_contacts` in `orchestrator/main.py`, a `contacts` lever through `resolve_config`/`load_config_from_resolved`, workspace write loops in `src/agent.py` / `src/api/persistent_session.py`, and a `contacts/` entry in `_LOOP_MAIN_GITIGNORE`. It was reverted the same day when this section was re-based onto [[virtual_directories]]. Only the **transport** died: its query body — resolve the job/session's project(s) → `get_project_contacts` → dedupe by contact id → render — became the internal endpoint the provider now calls, and `src/core/contact_files.py` survived the revert precisely so the provider could reuse it, which it does. The `README.md` index the module lacked lives in the provider.)*
...
- ~~Gated by `VIRTUAL_DIRS_ENABLED` (default `true`), shared with `tools/`~~ — superseded the previously planned `CONTACTS_MATERIALIZE_ENABLED`, and the gate itself was **removed 2026-08-07** (see [[virtual_directories]] §"No kill switch"). The overlay is unconditional; there is no flag.
