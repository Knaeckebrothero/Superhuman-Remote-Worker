...
**RESOLVED 2026-07-16 — moved to `done/`.** All three defects landed on `develop`
(verified as ancestors of HEAD): Track 1 model-picker/dispatch refactor (`da26eef1`
per-phase removal + migration `0039`, `2ebc97cd` effective-models, cockpit persist
UI-only) and Track 2 resilience (`f0b11887` C1+C2; C1 later refined so within-budget
cooldowns pause — `[[llm_cooldown_pause_and_resume]]`). C3 (stuck-detector visibility)
intentionally deferred. Unpushed at time of writing.

**Status:** Investigation complete, root causes isolated on the live main cluster. Three distinct defects, causally chained (A → B → C below). **Track 1 (P1 + P2) implemented + verified (local + k3d). Track 2 (Defect C): C1 (cooldown-aware fail-fast — **blanket fail-fast SUPERSEDED 2026-07-15: within-budget cooldowns now pause, `[[llm_cooldown_pause_and_resume]]`**) + C2 (consecutive-failure circuit breaker) implemented + unit/integration-verified; C3 deferred. All committed on `develop` (verified 2026-07-16)** — see the Implementation status notes under Sequencing / Track 2.
**Found:** 2026-06-27. Job `8bf2be7e-7ed3-4c7d-9a8b-4cdad339d69b` ("Loop iter 2 · CRITIC"), project **Better Resavio** (`0b24067c-1345-4e97-8614-3a950b1ba892`), owner `knaeckebrothero` / `operator@redacted.invalid`, main cluster.
...
**P2 implementation status (2026-06-27, uncommitted on `develop`):**
- ✅ **Layer 3 (backend)** — `GET /api/experts/{id}` now returns `effective_models` (per-slot `{model, source}`), computed by the pure `_effective_models_from_layers` helper (`main.py`) + `_compute_expert_effective_models` (account default_model + registry chat default), wired through `_load_expert_detail(..., user_id=)` and all three `get_expert` call sites. v1 source ∈ `{expert, account_default, system_default}` (project layer deferred — ~always null). Unit test `tests/test_effective_models.py` (8 cases).
- ✅ **Layer 3 (frontend)** — `model-group.component.ts` gains an `effectiveModels` input + `effectiveStrategic/Tactical/Session` + `strategicInEffect/…` (override > server effective > config fallback) and a `defaultLabel()` that renders the unset option as `Default · {model} ({source})`; the `<select>` now binds to the override signal so an untouched picker shows the labeled default instead of a blank/misleading option. Forwarded via `agent-settings.component.ts` and bound from `job-create`/`session-create` (`expertDetail()?.effective_models`). New i18n keys (`defaultResolved`, `defaultResolvedNoSource`, `source.*`) in `en` + `de-DE`. Spec: model-group "effective-model default label" + "model in effect".
- ✅ **Layer 4-frontend** — no new code: the Settings "Default Model"/"Auxiliary" dropdowns already bind `resolved().default_model`; they rendered blank only because the *backend* returned the YAML placeholder (not a dropdown option). The Layer 4-backend fix (registry value) makes the existing binding display correctly.
- Verified locally: `ruff`/`format` clean; `pytest` effective_models (8) + dispatch (28) + resolver/hydrate (16) + expert-access (20) pass; cockpit `vitest` model-group (22) + job-create (15) pass; `tsc --noEmit` clean; both i18n JSONs valid.
- Verified on **k3d** (`srw`): orchestrator serves the change — live `_load_expert_detail("scholar"/"defaults", user_id=test)` returned `effective_models` = registry default `gemma-4-moe` / source `system_default`; seeding the test user's `default_model=gpt-5.4-mini` flipped all slots to source `account_default` (cleaned up after). Running cockpit pod source carries the full frontend (effectiveModels input, `defaultLabel`, both create-form bindings, the type, i18n). Browser click-through of the rendered label still gated by login.
- **Still open:** Track 2 (Defect C circuit breaker) only.
...
**Track 2 implementation status (2026-06-27, uncommitted on `develop`):**
- ✅ **C1 — cooldown-aware 429** (`src/graph.py`): `_cooldown_reset_seconds` / `_cooldown_detail` detect a `model_cooldown` body (or a `reset_seconds` > `_COOLDOWN_MIN_RESET_SECONDS=300`); `_classify_llm_error` returns a new `cooldown` class (429 status branch, `RateLimitError` branch, and string fallback). The execute node fails fast on `cooldown` (`should_stop=True`, `recoverable=False`) with an operator-facing message naming the model + reset window — `determine_job_status` then marks the job `failed` (no orchestrator change). Resume after reset / pin a fallback.
  - **⚠️ SUPERSEDED 2026-07-15 by `[[llm_cooldown_pause_and_resume]]`** (implemented, committed on `develop`, unpushed): C1's *blanket* fail-fast was over-broad — real codex/OpenAI cooldowns reset in hours, not days. Now the execute node **pauses** a cooldown whose reset fits the 12h pause budget (routes it into the C-independent `llm_unavailable` outage path → resume-from-checkpoint), and **only fails fast** when the reset exceeds the budget (the original 5.5-day wall this incident was about) — so C1's intent survives exactly where it mattered. C2 (below) is untouched and still the backstop.
- ✅ **C2 — consecutive-failure circuit breaker** (`src/graph.py`): a closure counter `_llm_error_streak`, reset on any successful LLM response, increments on each execute invocation that exhausts its inner retries; at `_LLM_ERROR_STREAK_CAP=5` it hard-stops (`should_stop`, `recoverable=False`) instead of letting the outer graph loop re-enter `execute` forever. This is the deferred "Fix 3" from `[[agent_infinite_retry_on_permanent_llm_errors]]`, and it also bounds the previously-unbounded `auth_unavailable` retry loop (a positive side effect).
- ⏸️ **C3 — `get_stuck_jobs` visibility** deferred: C1+C2 stop the spin well inside the 30-min stuck-detection window, so the live-lock no longer occurs; C3 is now a low-value backstop for productive-looking loops (already partly covered by the graph fingerprint detector). Revisit only if a new live-lock class appears.
- Verified locally: `ruff`/`format` clean; `pytest` classifier+helpers `test_graph_helpers.py` (85, incl. 7 new cooldown cases) + execute-node/routing `test_graph.py` (66) pass; consolidated Track-1+2 backend run 185 pass.
- Verified on **k3d**: ran the **deployed agent image** (`srw-agent:tilt-2a8521153ec41f46`, what agents actually run) in a throwaway pod and called the live classifier — `_classify_llm_error` → `cooldown` for both the SDK-body and the stringified incident form, `rate_limit` for a plain 429 (no false positive), `_cooldown_detail` → `(482000.0, 'gpt-5.3-codex-spark')`, `_LLM_ERROR_STREAK_CAP == 5`. The successful import of `_cooldown_detail`/`_LLM_ERROR_STREAK_CAP` confirms the Tilt agent rebuild carries the change. (Same in-cluster function-call bar as P1/P2.)
- **Remaining (optional gold-standard):** a full live agent *run* that drives the execute node to `should_stop` — a real cooldown 429 can't be induced on demand; a C2 trip needs a broken-endpoint job + a ~2-3 min run. The execute-node `should_stop` routing is covered by `test_graph.py` (66).
