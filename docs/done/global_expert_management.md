...
| 9 | Enforcement points | Twice, like `can_use_vm` today: **save-time** (422 with actionable message; editor greys out ungated controls) and **dispatch-time** on the *full merged config* (expert fragment + project override + `users.settings.persistent_agent` + job/thread `config_override`) against the **runner's** grants. Dispatch check **rejects** (no silent stripping — silent capability downgrades burn debugging time). This single checkpoint also closes the `persistent_agent` self-service hole. **Amended 2026-08-04, save-time only, one route:** `POST /experts/{id}/duplicate` now **strips and reports** rather than rejecting, because rejecting was measured to block **7 of the 11 shipped experts** for a default-grants user — including `scholar`, which that route's docstring names as its purpose. Dispatch still rejects, so this row's main claim stands; and "no silent stripping" is preserved by the **report**, not by the rejection, which is what makes the `dropped` list load-bearing rather than cosmetic. The other four write routes still reject. See [[duplicate_expert_bypasses_user_experts_kill_switch]]. |
