...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Shipped + deployed `e65d5e32` — all seven recommendations verified at HEAD incl. the tested DelegationConfig plumbing; delegate_work granted nowhere anymore. Carve-out (separate gap, not a regression): delegation.enabled still does not gate spawn_subagent's tool BINDING (src/tools/delegation/spawn_subagent.py:239).
