...
**Listing cost (measured 2026-08-08, k3d).** Walking the mount one directory at a time cost **39.9 s for a 16-file tree**: `webdav3`'s `list()` issues a probe PROPFIND before the real one, so each directory is ~2.5 s against a server where a single PROPFIND is ~0.6 s. `WorkspaceSyncBase` now tries an optional `_list_remote_tree_fast()` first — Nextcloud answers the whole subtree from one `Depth: infinity` PROPFIND in **0.55 s**. The primitive defaults to `None` ("no such capability, walk as before"); since Nextcloud gates the feature behind `dav.propfind.depth_infinity`, any non-207 response flips a per-instance flag and the per-directory walk takes over permanently, so an unsupported server pays one probe once. Transport errors fall back for that call only, leaving the walk to surface the real error under the caller's `strict` rules. First-sighting local size checks and the push-side stat pass are likewise batched (or answered by one bulk backend listing where the backend offers it — see [[no_workspace_agent_mode]] §5.1).

**Turn-boundary means exactly one walk per boundary.** On the stateless lane the attach-time and teardown pulls were removed because each duplicated a pull that runs anyway: the turn-start pull already seeds the workspace seconds later, and the teardown pull refreshed a workspace whose next consumer always pulls first. Broken-mount surfacing lives in the turn's `_resilient_cloud_sync`, which already broadcasts `workspace_sync.error` and marks the session degraded — the same operator signal the attach-time pull's `strict=True` provided. The pinned lane keeps both pulls, since its workspace may be browsed after detach without another attach ever running.
