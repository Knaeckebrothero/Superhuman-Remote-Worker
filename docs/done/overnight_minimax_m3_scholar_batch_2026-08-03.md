...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Closed audit (5/5 jobs completed with git-backed reports); both named remediations shipped (arxiv/semantic-scholar client rewrite, PASS:/GAPS: transition templates). delegation.enabled non-enforcement re-confirmed still true and tracked via the tool-surface/registered-tools docs.
