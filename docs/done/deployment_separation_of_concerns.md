...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** A/B/D/E shipped (`fd07fb0b`, `3a2fbce7` et al.). C closed by REMOVAL, not by the proposed fix — the Mongo audit backend was replaced by the dedicated srw-auditdb Postgres; residue: values-experimental.yaml still toggles a dangling mongoExpress.enabled=true.
