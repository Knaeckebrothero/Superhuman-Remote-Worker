...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Shipped `812963ef` — audit_usage extraction + archiver capture + session threading all at HEAD; test_audit_usage covers it. Open question (whether CLIProxyAPI forwards cache stats upstream) is external — carry into any future cache-number dispute.
