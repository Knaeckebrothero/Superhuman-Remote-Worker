...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Superseded-but-resolved: the `_gw_target` carve-out described here was deleted with the whole LiteLLM gateway (`cce858e4`); the symptom (stale base_url/provider pinned on a codex model → 401) is fixed more generally — `_inject_model_credentials`/`_inject_dispatch_credentials` now unconditionally refresh transport from the endpoint row, pinned by TestEndpointDirectRouting + TestCodexSessionStaleTransport.
