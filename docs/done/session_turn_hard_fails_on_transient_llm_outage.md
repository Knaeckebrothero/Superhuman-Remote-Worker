...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** All three tracks shipped: Track 0 `ef12756a` (memory bump); Track 1 (image downscale tiers) `501f68bd`; Track 2 (pre-first-token transient session retry) `414059cb`, later folded into src/core/llm_retry.py. The doc's 'Tracks 1-2 proposed, not built' text simply predates them landing.
