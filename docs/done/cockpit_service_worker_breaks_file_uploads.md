...
**Status:** ✅ **RESOLVED — deployed + live-verified.** Fix committed `1195b54d` (2026-07-12), pushed to `develop`, rolled out via dev CI deploys (in `sha-7f60b5f` and later). **Live confirmation 2026-07-15:** real file upload on the dev cockpit succeeds with the new service worker active. Unit tests green (6 new interceptor specs + full 884-test suite); prod-build browser harness had verified the fixed path end-to-end beforehand (see investigation log).
...
- **2026-07-12** — Committed as `1195b54d` and pushed to `develop`; picked up by the dev CI deploy train (`sha-3a14ada` … `sha-7f60b5f`).
- **2026-07-15** — **Live-verified on the dev cluster:** real file upload through the cockpit succeeds with the deployed fix. Issue closed; doc moved to `docs/done/`.
