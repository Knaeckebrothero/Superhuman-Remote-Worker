...
**Status:** ✅ **RESOLVED 2026-06-26** — all 5 layers implemented + k3d-verified (1 provisioning pre-flight · 2 UI greying · 3 fail-loud-at-write-paths · 4 agent-side disambiguation · 5 default ceiling raised `supervised → auto_accept`). Phases 1–4 committed on `develop` (`5b05b6eb`…`66809484`); Phase 5 + this doc move in the closing commit. Was a **regression** since `a7ad2be0` (2026-06-18) that hit any **non-admin** picking `auto_accept`/`autonomous` (admins bypassed grants, which masked it). Root cause + layered design + per-phase verification below. Minor deferred follow-ups (none blocking): session **resume** path doesn't pre-flight (backstopped by Layer 4 at attach); cleaner agent-side *pre-apply* gate for mid-session changes.
...
- *Integrated k3d smoke test (✓):* drove the **real** `provision_or_assign` against a real `legacy1` (non-admin) autonomous thread on the live cluster → emitted `provisioning` → `failed{"config exceeds your capability grants: permission_mode: 'autonomous' exceeds the ceiling"}`, with `provision_agent`/`_find_idle` **not** called, no `srw-agent-s-*` pod spawned, thread left unbound (test thread cleaned up).
- *Not yet covered:* only the literal cockpit **browser** render of the `failed` card as a non-admin (pre-existing SSE plumbing — `failed{reason}` is already a handled cockpit state; needs a non-admin Keycloak login, which `legacy1` lacks). The session **resume** path (`POST …/resume`) shares the gap — follow-up (the incident was create + 425-fallback prepare, both now covered).

**Phase 2 — UI greying (Layer 1). ✅ DONE (develop, uncommitted; BROWSER-verified on k3d as a real non-admin).** ★ the "take away the option" UX. Independent of Phase 1.
- *Built:* `CapabilitiesService` (`core/services/capabilities.service.ts`, `providedIn:'root'`) fetches `GET /api/users/me/capabilities` once and exposes the resolved grants + `permissionModes()`/`allowsPermissionMode()`/`permissionRestricted()` via the existing `allowedEnumOptions` gate helper (admin/loading/error ⇒ all — fail-open, Phase 1 backstops). Wired into the permission-mode controls by their actual structure:
  - **New Session** (`/sessions/new` = `SessionCreateComponent` → `AgentSettingsComponent` → `ExecutionGroupComponent`): the execution group **already** filters `permission_mode`/`autonomy_ceiling` options by a `gatedCapabilities` input (the same mechanism the expert editor uses) — it just wasn't being fed. Added a `gatedCapabilities` input to `AgentSettingsComponent` that forwards to the execution group, and `SessionCreateComponent` now passes `capabilities.grants() ?? null`.
  - **Settings** (`settings.component.ts`) + **in-chat** (`persistent-chat.component.ts`): standalone selects → `[disabled]` modes above the ceiling via `capabilities.allowsPermissionMode(...)` + the existing `grants.locked.permission_mode` lock hint.
- *Course-correction (caught only by the browser test):* the first pass gated the inline `<app-select>` in `sessions-page.component.ts`, but that create dialog is **dead code** (`showCreate` is never set `true`; the real New-Session UI is the routed `SessionCreateComponent`). Reverted that edit + its spec mock. The unit tests + `ng build` + a live-grants logic-mirror had all passed because they verified the code I wrote, not whether it was the *rendered* control — only the browser caught the wrong target. (Lesson: for UI gating, render-level verification is not optional.)
- *Acceptance:* `supervised`-ceiling user → only `supervised`; admin → all three. ✓
- *Tests:* `capabilities.service.spec.ts` (4 cases: supervised / auto_accept / admin-null / error-fail-open). **92 cockpit tests pass; `ng build` clean.**
- *Verified (k3d, real browser via Playwright, real non-admin):* created a throwaway non-admin Keycloak user (`user` realm role ⇒ non-admin + auto-approved per `auth.py:269`), logged into `https://localhost/`. Endpoint returned `is_admin:false`, `permission_mode:"supervised"`. **New Session** permission dropdown rendered **only "Supervised"** (auto_accept/autonomous filtered out; `ExecutionGroupComponent.gatedCapabilities` confirmed populated). **Settings** rendered auto_accept/autonomous **`disabled`** + the lock hint. Screenshot captured; KC user deleted afterward. (in-chat uses the same verified `allowsPermissionMode` pattern as settings.)

**Phase 3 — fail loud at write paths (Layer 2). ✅ DONE (develop, uncommitted; e2e-verified on k3d).** Belt-and-suspenders for API/bypass/stale-UI.
- *Built:* `_enforce_session_create_grants(fragment, *, user_id, project_ids)` — a thin wrapper that runs the attach PDP (`_enforce_dispatch_grants`) and translates `GrantDenied → HTTP 422` (`_grant_violations_detail`). Called in **`create_thread`** (`POST /api/persistent/threads`) on the assembled `config_override` *before* persist, and in **`agent_update_thread_config`** (`PATCH …/config`) on the incoming override *before* persist (guarded: ownerless/standalone threads skip — `user_id NULL` isn't subject to a user's grants; admins bypass inside the PDP). Same wrapper is the remedy for [[mcp_created_jobs_ownerless_capability_grant_denied]].
- *Acceptance:* a create/update above the owner's ceiling → 422 with the violation; **nothing persisted**. Valid mode unaffected. ✓
- *Tests:* `test_capability_grants_api.py::test_enforce_session_create_grants_*` (denied→422 / admin-bypass / within-ceiling allowed). **675 py tests pass; ruff clean.**
- *Verified (k3d, real browser POST through the BFF):* as a non-admin (`phase2test`), `POST /api/persistent/threads {permission_mode:"autonomous"}` (with the cockpit's `X-CSRF:1` header) → **422** `"config exceeds your capability grants: permission_mode: 'autonomous' exceeds the ceiling"`; DB confirmed **no thread row created** (`DELETE 0` on cleanup — rejected before persist). KC user cleaned up.
- *Caveat:* the config-PATCH is the agent's persist step, so a denied mid-session change is rejected at persist (not before the agent applies it in-memory) — acceptable defense-in-depth since Phase 2 greys the chat dropdown and Phase 1 backstops at attach; the cleaner pre-apply gate is agent-side (future).

**Phase 4 — agent-side disambiguation (Layer 4). ✅ DONE (develop, uncommitted; unit + live-endpoint verified).** Safety net for the post-provision grant-revocation race.
- *Built:* new `SessionGrantDenied` exception (`orchestrator_client.py`). `get_thread_workspace(thread_id, *, raise_on_denied=False)` now raises it on a **403** (the only 403 the workspace endpoint emits — `require_internal` uses 401), carrying the violation, instead of collapsing to `None`; default stays `None` so the shared upgrade/VM pollers are unaffected. `_poll_workspace_ready` gained `raise_on_denied` (threaded only by the attach caller, 1108) and propagates the denial (no retry). The attach lifespan catches `SessionGrantDenied` → new `_exit_grant_denied`, which logs the REAL reason at ERROR (not "No workspace container provisioned" / "will rebind once the workspace recovers") and exits 0; the cockpit re-surfaces it on its next create/prepare via the Layer-1/2 pre-flight.
- *Why last:* Phase 1 stops the doomed pod from ever being spawned, so this only covers a grant **revoked between provision and the agent's workspace fetch** (~2 s window) — rare, hence lower urgency.
- *Tests:* `test_orchestrator_client.py::test_raises_session_grant_denied_on_403_when_flagged` + `…_returns_none_on_403_by_default` (backward compat); `test_persistent_app.py::TestPollWorkspaceReady::test_propagates_grant_denied_when_flagged`. **1045 py tests pass; ruff clean.**
- *Verified (k3d, live):* the real workspace endpoint, queried with the internal key for a denied (`legacy1` + autonomous) thread, returns **403** `"config exceeds your capability grants: permission_mode: 'autonomous' exceeds the ceiling"` — the exact 403+detail shape the unit-tested `get_thread_workspace(raise_on_denied=True)` parses into `SessionGrantDenied`. (Triggering the full live race is impractical — Phase 1 prevents the normal denied path from ever reaching the agent — so the two sides of the contract are verified independently: live endpoint 403 + unit-tested client handling.)

**Phase 5 — policy (Layer 5). ✅ DONE (develop, uncommitted; unit + live-verified).** Decision (operator call, 2026-06-26): **raise the default `permission_mode` ceiling `supervised → auto_accept`** rather than per-user grants or a grandfather migration. One-line catalog change in `src/core/capability_grants.py` (+ the `evaluate()` fallback): every approved user may pick **supervised + auto_accept** without a grant; **autonomous** (fully unattended) still requires an explicit grant. Safe because agents run in **isolated sandbox workspaces** and shell execution is **separately gated by `shell_tools`**; a global-scope grant can restrict back to supervised if ever needed. The whole layered stack (Phases 1–4 + cockpit greying) adapts automatically — they all read the resolved ceiling.
- *Tests:* `test_capability_grants.py` (default = auto_accept; auto_accept now passes the session gate, autonomous still gated; `DEFAULTS` is derived from `CATALOG`, so the gating tests auto-adapt). **215 grant/expert tests pass; ruff clean.**
- *Verified (k3d, live):* `CATALOG.permission_mode.default == "auto_accept"`; `resolve_grants_for(legacy1)` (non-admin, no grant) → ceiling `auto_accept`; create-grant check: supervised ✓ / auto_accept ✓ / autonomous → 422.
- *Side effects / notes:* mivan's explicit `auto_accept` user grant is now **redundant** (he gets it by default) — harmless to leave. The **selected-mode** default stays `supervised` (loader/mcp/persistent_session/orchestrator_client) — sessions still default to a supervised *selection*; users may now *pick* auto_accept. **Prod:** this default applies on prod too once `EXPERTS_DB_ENABLED` flips — deliberate; seed `permission_mode: autonomous` grants for anyone needing unattended mode as part of that enablement.
