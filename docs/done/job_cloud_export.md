...
**Verification:** 70 loop unit tests green, ruff clean, guard confirmed live on the k3d pod. Sweeper self-heal live-verified: flipping the wedged job to `completed` made the sweeper re-run the advance and rotate the loop cleanly to `critic` — confirming `pending_review` was the sole blocker. **Execution-role live check passed 2026-07-07:** the smoke loop rotated through `scholar → product-qa → critic → developer`; the `developer` job (`3ce2f836`), which carried a `cloud_diff_baseline_commit`, landed **`completed`, not `pending_review`**, and `diff_status` was never set — the Mode-A block was skipped for the loop job exactly as the `and not job_loop_id(job)` guard intends. The loop advanced developer → done cleanly.
