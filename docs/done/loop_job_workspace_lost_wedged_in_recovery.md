...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Resolved as the doc records — G1 backend-aware recovery routing + bounded recovery_attempts + the stable-DNS reattach fix all verified at HEAD; residual concerns split into their own open docs.

**Status:** Investigated on the live main cluster + a 5-agent code audit (2026-06-29). The recovery state machine is now **fully traced at code level** and the mechanism **confirmed** (this supersedes the earlier "to be confirmed / events aged out" framing — several of those guesses were wrong; corrections are called out inline). **Wedge effectively RESOLVED 2026-06-29** (per the Update note directly below — the PVC-reattach feature: G1 bounded `recovery_attempts` → fail-loud + auto-resume; supersedes the earlier "not fixed" framing that the rest of this line still reflected). Two concerns were split out and **remain open**: the *upstream trigger* (why the workspace pod died) → `agent_workspace_pod_resource_headroom.md`; the finding that snapshot/restore is *structurally dead for ALL jobs* → `snapshot_restore_dead_for_jobs.md`.
...
**Found:** 2026-06-27. Job `19707fa1-1788-4eda-a296-8b108429b108` (Loop iter 3 · DEVELOPER — "Build an ERP for Hotel Rheinland in Bad Orb"), project `68137e29-6b1f-4f1b-a0c1-4e6dc2be3f9a`, owner `knaeckebrothero` / `overlygenericaddress@pm.me`, main cluster (ns `superhuman-remote-worker`). Still `paused` and inert as of 2026-06-29.
