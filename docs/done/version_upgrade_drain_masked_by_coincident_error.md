...
**Status:** RESOLVED — shipped in commit `4ff91c7c` (2026-07-07), on `develop`, live on the main cluster (orchestrator image `sha-4ff91c7`; both replicas verified). Investigated 2026-07-07 from two failed Better-Resavio loop iterations; root cause confirmed by elimination against the DB state + code paths. Orchestrator guard (`completion.py`) + agent-side error clear (`graph.py`) + 7 tests (255 passing across affected suites; ruff clean). Verified live on k3d 2026-07-07 via the `/complete` endpoint drill: the exact incident payload `{version_upgrade freeze + coincident error}` → `paused` + freeze stashed to `context.last_freeze_data` + `freeze_data` column NULL + guard `WARNING` logged; control `{bare error}` → `failed`. The full-agent live-drain drill was intentionally skipped (no worker agents on that k3d; it would exercise only the happy-path clean drain, which the endpoint drill + the real-node unit test in `test_graph.py` already cover). Post-deploy: no new version_upgrade-masked failures reported. The exact `error_message` on the two original jobs was never retrieved (moot; see "Open questions"). Symbols/line numbers current as of 2026-07-07.
...
# resumes it). docs/done/version_upgrade_drain_masked_by_coincident_error.md
