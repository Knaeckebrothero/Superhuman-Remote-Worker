...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Shipped `7fb9e9e2` — stable headless Service + _workspace_dns() at the single ready-context write point serving both dispatch and resume.

**Status:** Filed + diagnosed live on k3d (2026-06-29). **FIXED via Option 1 (stable headless Service) — committed `7fb9e9e2`; full real-job auto-resume E2E PASSED 2026-06-29 (acceptance met).** Each PVC-backed job workspace now gets a headless Service named after the pod, so the agent dials a constant DNS (`workspace-<id>.<ns>.svc:30022`) that always resolves to the current pod; both the dispatch and resume paths prefer this stable `host` over the ephemeral `pod_ip` (resume previously injected no container host at all → used the stale persisted IP).
**E2E (real scholar job `b4025433`, all of G1+G2+Option 1 live):** killed the running job's workspace pod mid-run → agent (dialing the stable DNS) exhausted its reconnect loop → G1 re-dispatched via the **pod arm** (`vm.requested` null, no VM misroute) → new pod `10.42.0.107→.110` reattached the PVC + got the same headless Service (same DNS) → `_resume_job_on_agent` injected the stable DNS host → **new agent connected to the reattached workspace, resumed from the Postgres checkpoint** ("Graceful resume from 'paused'", reloaded 3 todos), the **un-pushed sentinel survived** (G2 preserve, no `rm -rf`), and the job returned to `processing` (NOT failed, recovery cap not consumed).
**Follow-up (minor, non-blocking):** recovery took ~3.5 min, dominated by the agent's ~2.7 min SSH reconnect loop (`_max_retries=5` × 5 sub-attempts) retrying the endpoint-less DNS *before* giving up to trigger the recreate. With a stable DNS the pod isn't recreated until the agent reports `workspace_unavailable`, so that retry time is wasted. Options to shorten: lower the workspace-backend reconnect cap, or have the lifecycle drift-probe proactively recreate the dead pod so the agent's in-process reconnect succeeds without a full re-dispatch.
