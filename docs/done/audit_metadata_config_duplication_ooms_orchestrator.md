...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Write-side denylist `7ea0d798` (tested) + read-side lean projection + bulk-endpoint deletion + MCP tools re-plumbed onto lean/paged endpoints (clamp 200) + Mongo backend deleted outright (`f1b06621`). Open residue: existing-row backfill — disk cleanup, not a crash vector.
