...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Primary fix (_resolve_max_output_tokens at 8 provider sites + oversized-message backstop) and recurrence items B/C/D/E all committed (D's probe timeouts included). Residuals A (original crash root cause), F (codex-proxy max_tokens confirm), G (summary quality) remain open but none block the fixed defect.
