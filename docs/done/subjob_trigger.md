...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** The doc's own SUPERSEDED banner verified accurate — the refactor roadmap is fully implemented server-side (/api/jobs/{id}/complete, _trigger_verification_on_complete, _handle_critic_verdict_on_complete, determine_job_status), with an in-code comment confirming the replacement.
