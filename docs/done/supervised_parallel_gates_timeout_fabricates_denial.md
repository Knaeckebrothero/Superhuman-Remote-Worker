...
**Status:** **Scope A SHIPPED — committed `54e42626`, deployed dev `sha-f0cd0e0`, LIVE GATE PASSED 2026-07-29** (evidence below). Root cause confirmed via live reproduction on the dev cluster. Fix 1 (three-state gate, park instead of fabricating a denial, tethered wait, interrupt-cancellable) + Fix 2 (pending gates re-surface on attach) are done with tests; see "Implementation" below.

**Follow-on (2026-08-09) — the deferred "batch-approval UX" item is now also SHIPPED and live-gated.** One card lists every call in a parallel batch, decided with one click; execution stays sequential. Design `docs/superpowers/specs/2026-08-01-batch-tool-approval-design.md`, plan `docs/superpowers/plans/2026-08-01-batch-tool-approval.md`, residuals `docs/issues/batch_tool_approval_residuals.md`.

That work briefly **re-created this very bug in the UI**: the new turn-end sweep broadcasts `permission.resolved` with `decision: "expired"`, and the cockpit was collapsing every non-`approved` decision to `'denied'` — so an ordinary parked turn painted un-reached calls as explicitly refused, while the backend was correct. Fixed in `d6518b32`: `permission.resolved` maps three ways and `'expired'` is its own tool-call status (settled so it does not spin, never counted as denied, rendered "not run", and fabricating no row when there is no matching call). **Lesson worth keeping: a backend that stops lying is not enough — any client that collapses its three-state outcome back to a boolean reintroduces the same false denial.**

Three further defects in that work (a claim/sweep race, a ledger entry dropped when the DB is unreachable, and a cross-thread ledger leak) were fixed in `69e22f0c` and verified in-pod on k3d against real Postgres (7/7 checks).

**Follow-on (2026-08-16) — the last fabricated denial, on the error path.** The TTL stopped lying in `54e42626`, but `_wait_for_permission_resolution` still ended with `except Exception: return "denied"`, and two sibling defaults did the same (a session that died mid-wait, and a CAS-expire whose row had been retired out from under it). Any of them told the model *"User declined this tool call."* over an infrastructure failure — worse than the TTL case, because the row stays `pending`, so the DB flatly contradicts the refusal the transcript claims. Found while diagnosing a CI flake in which a leaked cross-loop `asyncio.Event` made the wait raise on every call: two of the three tests still passed, because one of them *asserted* the fabricated denial. Now these return non-decisions (`'unavailable'`, `'expired'`), the caller's existing three-state mapping parks the turn (`NO_ANSWER`), and only an explicit `'denied'` is ever reported as a refusal. **Only 'denied' means the user said no** is now the invariant the docstring states and the tests pin.

That same change closed the reload **"completed" mislabel** for gated calls (the last open item below): history hydration mapped every decision except `denied`/`expired` to `completed` — the mirror image of the original bug, claiming a tool ran when the gate had actually settled `interrupted` (Stop) or `unavailable`. It now normalizes the wire value exactly like the live `permission.resolved` path: only an approved or un-gated call renders completed, only `denied` renders denied, every other non-decision renders "not run". **Second time the client-side collapse bit — the lesson above generalizes: any place that maps this vocabulary to a UI state needs a safe default, in both directions.**

Still open from the original "out of scope" list: the **`425 /connection` stream storm** (see `resumed_session_dead_stream_and_supervised_gate_timeout_as_denial.md`). The reload "completed" mislabel is closed for supervised calls by the 2026-08-16 follow-on above.
**Found:** 2026-07-24 (user report: *"When the AI sends multi tool calls but is on supervised we can only approve one — the rest fails"*). Root cause confirmed via live repro 2026-07-25, session `83dc7f7a-75b0-4141-ace6-0c5413a3e5cf` (dev `cockpit.srw.works`, user `overlygenericaddress@pm.me`, model `MiniMax-M3`).
