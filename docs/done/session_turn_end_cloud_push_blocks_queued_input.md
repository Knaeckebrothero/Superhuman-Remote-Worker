...
**Closed by the 2026-08-06 doc-truth sweep (batch #3):** Shipped `99b87008` — turn-end push off the critical path (background task), remote-tree dedup seeding, cockpit isAwaitingTurn, UUID-validated get_thread; tests/test_turn_end_cloud_push_async.py 6/6. Session-latency smoke on a deployed dev cluster still owed (branch since pushed mid-sweep; the smoke itself has not run).
