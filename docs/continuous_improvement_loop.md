...
| `get_job_file` | `GET /api/jobs/{id}/repo/file?path=…&ref=…` | Read plan.md, notes/ from another job's Gitea repo (committed state as of its last push) |
...
- [[finetuning]] - Training data generated by the evaluator feeds into model finetuning
- [[debug_cockpit]] - Cockpit UI for inspecting job audit trails the evaluator analyzes
- [[deployment]] - Pipeline script runs alongside the deployed agent stack
- [[agent_improvements]] - Improvements identified and implemented by the CI loop
- [[context_management]] - Evaluator analyzes context management effectiveness per job
