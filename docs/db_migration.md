...
The gate only works if a fresh regeneration is a pure function of the migration chain, so the normalizer strips everything the dump carries that the migrations don't determine: the random `\restrict` token, the pg_dump version header, and — the non-obvious one — the **rolling monthly audit partitions**. The audit migrations seed those relative to `now()` (current month + 2 lookahead), so replaying the same chain in July produces `_p2026_07..09` and in August `_p2026_08..10`. Left alone that fires the gate for everyone on the 1st of every month with a diff that is pure calendar and zero schema. The normalizer rewrites the window onto a fixed synthetic epoch, which is why `audit_schema_current.sql` shows `agent_audit_p1970_01..03` and 1970 partition bounds. **Those months are not real** — only the leaf count, bound width, storage params, indexes and constraints carry meaning; live databases carry the actual months. If you add another now()-relative object to a migration, either canonicalize it the same way or the gate becomes a monthly false alarm again.
