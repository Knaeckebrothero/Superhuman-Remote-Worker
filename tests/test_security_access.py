...
# =============================================================================
# require_builder_session_owner — personal scratch, no project sharing
# =============================================================================


class TestRequireBuilderSessionOwner:
    @pytest.mark.asyncio
    async def test_owner_passes(self, user_a, builder_session_a, fake_db, fake_request):
        with _patch_caller(user_a):
            user, session = await access.require_builder_session_owner(
                fake_request, fake_db, str(builder_session_a["id"])
            )
        assert user is user_a
        assert session is builder_session_a
    ...
    @pytest.mark.asyncio
    async def test_admin_bypasses(
        self, user_admin, builder_session_a, fake_db, fake_request
    ):
        with _patch_caller(user_admin):
            user, session = await access.require_builder_session_owner(
                fake_request, fake_db, str(builder_session_a["id"])
    ...
    @pytest.mark.asyncio
    async def test_missing_session_404(self, user_a, fake_db, fake_request):
        with _patch_caller(user_a):
            with pytest.raises(HTTPException) as exc:
                await access.require_builder_session_owner(
                    fake_request,
    ...
    @pytest.mark.asyncio
    async def test_orphan_session_admin_only(
        self, user_a, user_admin, fake_db, fake_request
    ):
        """An orphan session (user_id=NULL) should fail-closed for normal users."""
        orphan = {
            "id": UUID("0badbeef-0bad-0bad-0bad-0badbeef0bad"),
            "user_id": None,
            "expert_id": None,
        }
        fake_db.get_builder_session = AsyncMock(return_value=orphan)
        with _patch_caller(user_a):
            with pytest.raises(HTTPException) as exc:
                await access.require_builder_session_owner(
                    fake_request, fake_db, str(orphan["id"])
                )
        assert exc.value.status_code == 403
        # Admin still passes
        with _patch_caller(user_admin):
            _, session = await access.require_builder_session_owner(
                fake_request, fake_db, str(orphan["id"])
            )
        assert session is orphan
