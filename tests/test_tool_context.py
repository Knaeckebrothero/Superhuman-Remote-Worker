...
class TestUpdateJobStatus:
    """Tests for async update_job_status method."""

    @pytest.mark.asyncio
    async def test_raises_without_job_id(self):
        """Should raise ValueError when no job_id available."""
        ctx = ToolContext()
        with pytest.raises(ValueError, match="No job_id"):
            await ctx.update_job_status("completed")

    @pytest.mark.asyncio
    async def test_returns_false_without_postgres(self):
        """Should return False when no postgres_db."""
        ctx = ToolContext(_job_id="job-1")
        result = await ctx.update_job_status("completed")
        assert result is False

    @pytest.mark.asyncio
    async def test_status_only_update(self):
        """Should execute status-only SQL."""
        db = AsyncMock()
        ctx = ToolContext(_job_id="job-1", postgres_db=db)
        result = await ctx.update_job_status("running")
    ...
    @pytest.mark.asyncio
    async def test_status_with_completed_at(self):
        """Should include completed_at when requested."""
        db = AsyncMock()
        ctx = ToolContext(_job_id="job-1", postgres_db=db)
        result = await ctx.update_job_status("completed", completed_at=True)
        assert result is True
        sql = db.execute.call_args[0][0]
        assert "completed_at = NOW()" in sql

    @pytest.mark.asyncio
    async def test_status_with_error_message(self):
        """Should include error_message when provided."""
        db = AsyncMock()
        ctx = ToolContext(_job_id="job-1", postgres_db=db)
        result = await ctx.update_job_status(
            "failed", completed_at=True, error_message="boom"
        )
        assert result is True
        sql = db.execute.call_args[0][0]
        assert "error_message = $2" in sql

    @pytest.mark.asyncio
    async def test_returns_false_on_exception(self):
        """Should return False on database exception."""
        db = AsyncMock()
