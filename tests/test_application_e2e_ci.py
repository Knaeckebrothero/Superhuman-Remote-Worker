...
def test_application_e2e_is_a_separate_scheduled_manual_observation_lane() -> None:
    ...
    assert "./scripts/e2e-app.sh run" in raw
...
def test_workflow_pins_the_harness_toolchain_and_keeps_a_teardown_reserve() -> None:
    ...
    assert "timeout --foreground --signal=INT --kill-after=4m 38m" in raw
