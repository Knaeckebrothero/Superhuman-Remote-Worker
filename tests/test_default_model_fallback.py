"""Tests for ``PostgresDB.resolve_default_for_capability``.
...
catalog row is available. Every catalog capability — chat, auxiliary,
embedding, vision, whisper, tts — goes through the same validation path.
"""
...
def _db(
    *,
    ...
    """Build a stub PostgresDB exposing only the methods
    ``resolve_default_for_capability`` consults. Avoids spinning up an actual
    connection pool."""
...
async def test_returns_pin_when_pin_resolves_to_enabled_catalog_row():
    ...
    assert await db.resolve_default_for_capability("auxiliary") == "claude-opus-4-7"
    db.list_models_by_capability_alphabetical.assert_not_awaited()
...
async def test_returns_alphabetical_first_when_pin_is_missing_from_catalog():
    ...
    assert await db.resolve_default_for_capability("auxiliary") == "alpha-model"
...
async def test_returns_none_when_no_pin_and_no_catalog_rows():
    ...
    assert await db.resolve_default_for_capability("auxiliary") is None


@pytest.mark.asyncio
async def test_each_catalog_capability_resolves_independently():
    db = _db(
        pin=None,
...
async def test_whisper_pin_validates_against_catalog():
    ...
    ``resolve_catalog_model`` like every other capability."""
    db = _db(
        pin="whisper-1",
        catalog_resolves_to={"model_id": "whisper-1", "enabled": True},
    )
    assert await db.resolve_default_for_capability("whisper") == "whisper-1"
    db.resolve_catalog_model.assert_awaited_once_with("whisper-1", capability="whisper")
...
async def test_tts_returns_none_when_no_pin_and_no_catalog_rows():
    ...
    contract as the other catalog capabilities."""
    db = _db(pin=None, alphabetical=[])
    assert await db.resolve_default_for_capability("tts") is None
