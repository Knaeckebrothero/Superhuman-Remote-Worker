...
class TestRedactThreadMetadataShape:
    ...
    def test_string_metadata_returned_as_parsed_object(self):
        ...
        out = tp_routes._redact_thread_metadata({"id": "t", "metadata": json.dumps(md)})
    ...
    def test_dict_metadata_stays_object_and_redacts(self):
        ...
        out = tp_routes._redact_thread_metadata({"id": "t", "metadata": md})
        assert out["metadata"]["datasource_ids"] == ["a"]
        assert "api_key" not in out["metadata"]["config_override"]["llm"]

    def test_absent_or_unparseable_metadata_becomes_empty_object(self):
        assert tp_routes._redact_thread_metadata({"id": "t"})["metadata"] == {}
        assert (
            tp_routes._redact_thread_metadata({"id": "t", "metadata": "{not json"})[
                "metadata"
            ]
            == {}
        )
        assert (
            tp_routes._redact_thread_metadata({"id": "t", "metadata": None})["metadata"]
            == {}
        )

    def test_workspace_binding_dropped(self):
        out = tp_routes._redact_thread_metadata(
            {
                "id": "t",
                "metadata": {
    ...
    def test_runtime_retirement_authority_is_redacted_to_safe_state(self):
        ...
        out = tp_routes._redact_thread_metadata({"id": "t", "metadata": {}, **internal})
        for key in internal:
