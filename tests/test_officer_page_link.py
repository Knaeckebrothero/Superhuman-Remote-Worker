...
from orchestrator.services.officer_paging import (
...
class TestOfficerSessionLink:
    def test_env_set_builds_link(self, monkeypatch):
        ...
        assert officer_session_link(THREAD_ID) == f"{BASE}/sessions/{THREAD_ID}"
    ...
    def test_env_unset_returns_none(self, monkeypatch):
        ...
        assert officer_session_link(THREAD_ID) is None
...
@pytest.fixture
def paging(record) -> OfficerPagingDependencies:
    """The notifier is a dependency field now — the recorder is wired here
    instead of onto ``orchestrator.main.notification_service``."""
    return OfficerPagingDependencies(
        store=MagicMock(), notifier=MagicMock(record=record)
    )
...
class TestDispatchOfficerPageLink:
    ...
    async def test_appends_link_when_base_url_set(
        self, thread, record, paging, monkeypatch
    ):
        monkeypatch.setenv("COCKPIT_EXTERNAL_URL", BASE)
...
class TestDispatchOfficerPageRecords:
    ...
    @pytest.mark.asyncio
    async def test_default_subject(self, thread, record, paging):
        notification_id = await dispatch_officer_page(
            thread, THREAD_ID, "", "Wake up.", dependencies=paging
        )
        assert notification_id == "n-1"
        assert record.call_args.kwargs["subject"] == "Your centurion needs you"

    @pytest.mark.asyncio
    async def test_dedup_key_collapses_identical_text_on_one_day(
        self, thread, record, paging
    ):
        await dispatch_officer_page(
            thread, THREAD_ID, "S", "Same text", dependencies=paging
        )
        await dispatch_officer_page(
            thread, THREAD_ID, "S", "Same text", dependencies=paging
        )
        await dispatch_officer_page(
            thread, THREAD_ID, "S", "Other text", dependencies=paging
        )
    ...
    @pytest.mark.asyncio
    async def test_explicit_category_and_key_are_passed_through(
        self, thread, record, paging
    ):
        notification_id = await dispatch_officer_page(
            thread,
    ...
    @pytest.mark.asyncio
    async def test_low_severity_for_digest(self, thread, record, paging):
        await dispatch_officer_page(
            thread, THREAD_ID, "S", "M", severity="low", dependencies=paging
        )
        assert record.call_args.kwargs["severity"] == "low"

    @pytest.mark.asyncio
    async def test_no_owner_means_nobody_to_notify(self, record, paging):
        notification_id = await dispatch_officer_page(
            {"user_id": None}, THREAD_ID, "S", "M", dependencies=paging
        )
        assert notification_id is None
        record.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_feed_write_failure_is_reported_not_raised(
        self, thread, record, paging
    ):
        record.side_effect = RuntimeError("pool down")
        notification_id = await dispatch_officer_page(
            thread, THREAD_ID, "S", "M", dependencies=paging
        )
        assert notification_id is None
