...
@pytest.fixture
def dependencies(db):
    return contacts_router.ContactsDependencies(db=db)


@pytest.fixture
def as_user_a(monkeypatch, dependencies):
    monkeypatch.setattr(
        dependencies, "require_approved_user", AsyncMock(return_value=USER_A)
    )


async def test_list_scopes_to_caller(db, as_user_a, dependencies):
    db.list_contacts_for_user.return_value = []
    out = await contacts_router.list_contacts(
        _req(), project_id=None, channel=None, q=None, dependencies=dependencies
    )
...
async def test_patch_contact_owner_only(db, as_user_a, dependencies):
...
async def test_add_address_validates_and_409s(db, as_user_a, dependencies):
...
async def test_link_requires_editor_and_visibility(
    db, as_user_a, monkeypatch, dependencies
):
    gate = AsyncMock()
    monkeypatch.setattr(dependencies, "require_project_member", gate)
    db.user_can_see_contact.return_value = False
    with pytest.raises(HTTPException) as e:
        await contacts_router.link_contact_to_project(
            _req(), "c1", "p1", dependencies=dependencies
        )
    assert e.value.status_code == 404
    gate.assert_awaited()  # editor gate ran before visibility


async def test_delete_contact_owner_only(db, as_user_a, dependencies):
    ...
    out = await contacts_router.delete_contact(_req(), "c1", dependencies=dependencies)
    assert out == {"status": "deleted"}


async def test_link_extracts_user_from_member_tuple(
    db, as_user_a, monkeypatch, dependencies
):
    ...
    out = await contacts_router.link_contact_to_project(
        _req(), "c1", "p1", dependencies=dependencies
    )
...
async def test_unlink_contact_from_project(db, as_user_a, monkeypatch, dependencies):
    gate = AsyncMock(return_value=(USER_A, {"id": "p1"}))
    monkeypatch.setattr(dependencies, "require_project_member", gate)

    db.unlink_contact_from_project.return_value = False
    with pytest.raises(HTTPException) as e:
        await contacts_router.unlink_contact_from_project(
            _req(), "c1", "p1", dependencies=dependencies
        )
    assert e.value.status_code == 404
    ...
    out = await contacts_router.unlink_contact_from_project(
        _req(), "c1", "p1", dependencies=dependencies
    )
    assert out == {"status": "unlinked"}


async def test_patch_address_not_found(db, as_user_a, dependencies):
    db.get_contact_address.return_value = None
    with pytest.raises(HTTPException) as e:
        await contacts_router.patch_address(
            _req(), "a1", contacts_router.AddressPatch(), dependencies=dependencies
        )
    assert e.value.status_code == 404


async def test_patch_address_owner_only(db, as_user_a, dependencies):
    ...
    with pytest.raises(HTTPException) as e:
        await contacts_router.patch_address(
            _req(),
            "a1",
            contacts_router.AddressPatch(address="x@y.com"),
            dependencies=dependencies,
        )
    assert e.value.status_code == 403
...
async def test_patch_contact_blank_display_name_400(db, as_user_a, dependencies):
    ...
    with pytest.raises(HTTPException) as e:
        await contacts_router.patch_contact(
            _req(),
            "c1",
            contacts_router.ContactPatch(display_name="   "),
            dependencies=dependencies,
        )
    assert e.value.status_code == 400
    db.update_contact.assert_not_awaited()
...
async def test_patch_contact_strips_display_name(db, as_user_a, dependencies):
    db.get_contact.return_value = {"id": "c1", "owner_user_id": USER_A["id"]}
    db.update_contact.return_value = {"id": "c1", "display_name": "Anna"}
    out = await contacts_router.patch_contact(
        _req(),
        "c1",
        contacts_router.ContactPatch(display_name="  Anna  "),
        dependencies=dependencies,
    )
...
async def test_patch_address_duplicate_409(db, as_user_a, dependencies):
    ...
    with pytest.raises(HTTPException) as e:
        await contacts_router.patch_address(
            _req(),
            "a1",
            contacts_router.AddressPatch(address="dupe@acme.de"),
            dependencies=dependencies,
        )
    assert e.value.status_code == 409
...
async def test_patch_address_normalizes_on_success(db, as_user_a, dependencies):
    db.get_contact_address.return_value = {
        "id": "a1",
        "owner_user_id": USER_A["id"],
        "channel": "email",
...
async def test_delete_address_owner_only(db, as_user_a, dependencies):
    db.get_contact_address.return_value = {"id": "a1", "owner_user_id": USER_B["id"]}
    with pytest.raises(HTTPException) as e:
        await contacts_router.delete_address(_req(), "a1", dependencies=dependencies)
    assert e.value.status_code == 403
    db.delete_contact_address.assert_not_awaited()


async def test_delete_address_success(db, as_user_a, dependencies):
    db.get_contact_address.return_value = {"id": "a1", "owner_user_id": USER_A["id"]}
    out = await contacts_router.delete_address(_req(), "a1", dependencies=dependencies)
    db.delete_contact_address.assert_awaited_once_with("a1")
    assert out == {"status": "deleted"}


async def test_add_address_unknown_channel_400(db, as_user_a, dependencies):
    db.get_contact.return_value = {"id": "c1", "owner_user_id": USER_A["id"]}
    with pytest.raises(HTTPException) as e:
        await contacts_router.add_address(
            _req(),
...
async def test_create_contact_success_adds_addresses(db, as_user_a, dependencies):
    ...
    out = await contacts_router.create_contact(_req(), body, dependencies=dependencies)
...
async def test_create_contact_rolls_back_on_duplicate_address(
    db, as_user_a, dependencies
):
    db.create_contact.return_value = {"id": "c1"}
    ...
    with pytest.raises(HTTPException) as e:
        await contacts_router.create_contact(_req(), body, dependencies=dependencies)
    assert e.value.status_code == 409
    db.delete_contact.assert_awaited_once_with("c1")


async def test_project_get_contacts_gated(db, monkeypatch, dependencies):
    gate = AsyncMock(return_value=USER_A)
    monkeypatch.setattr(dependencies, "require_project_member", gate)
    db.get_project_contacts.return_value = []
    out = await contacts_router.get_project_contacts(
        _req(), "p1", dependencies=dependencies
    )
    gate.assert_awaited()
    assert out == {"contacts": []}


async def test_project_post_find_or_create_then_link(db, monkeypatch, dependencies):
...
async def test_project_post_create_rolls_back_on_duplicate_address(
    db, monkeypatch, dependencies
):
    monkeypatch.setattr(
        dependencies,
        "require_project_member",
