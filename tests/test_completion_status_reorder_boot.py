...
async def test_reorder_without_completion_commands_fails_before_db_connect(
    ...
    monkeypatch.setattr(orchestrator.main, "COMPLETION_STATUS_REORDER_ENABLED", True)
    connect = AsyncMock()

    with patch.object(orchestrator.main.postgres_db, "connect", connect):
        with patch("orchestrator.main.sys.exit", side_effect=SystemExit) as exit_mock:
            with pytest.raises(SystemExit):
                async with orchestrator.main.lifespan(MagicMock()):
                    pass
