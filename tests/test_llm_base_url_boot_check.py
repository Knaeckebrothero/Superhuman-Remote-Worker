...
async def test_lifespan_exits_when_llm_base_url_set(caplog):
    ...
    with patch.dict(os.environ, {"LLM_BASE_URL": "http://stale-vllm:8080/v1"}):
        with patch("orchestrator.main.sys.exit", side_effect=SystemExit) as mock_exit:
            with patch.object(orchestrator.main.postgres_db, "connect", AsyncMock()):
...
async def test_lifespan_proceeds_when_llm_base_url_unset():
    ...
    with patch.dict(os.environ, env, clear=True):
        with patch("orchestrator.main.sys.exit") as mock_exit:
