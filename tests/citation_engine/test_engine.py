...
class TestSourceRegistration:
    ...
    def test_add_custom_source(self, engine):
        ...
        assert (
            source.metadata["description"] == "Computed analysis of paper architectures"
        )
    ...
    def test_list_sources(self, engine, sample_text_file):
        ...
        engine.add_db_source(
            identifier="db1", name="DB 1", content="database test content"
        )
...
class TestCitationCreation:
    ...
    @patch.object(CitationEngine, "_verify_citation")
    def test_citation_reasoning_required(
        self, mock_verify, engine, sample_text_file, monkeypatch
    ):
...
class TestExport:
    ...
    def test_format_citation_harvard(self, mock_verify, engine, sample_text_file):
        ...
        source = engine.add_doc_source(
            sample_text_file, name="Test Doc", version="2024"
        )
        result = engine.cite_doc(
            claim="Test claim",
            source_id=source.id,
