"""
PostgreSQL integration tests for the vendored citation_engine (multi-agent mode).

Skipped by default; run only when RUN_POSTGRES_TESTS=true. They exercise a real
citation round-trip (register source -> cite -> retrieve) against a live Postgres.

IMPORTANT: the engine no longer creates its own schema in Postgres mode — the host
application owns it (see CitationEngine._initialize_schema). So these must run
against a database where SRW's vector migrations have already applied the schema,
i.e. the k3d/dev pgvector instance's ``citation_engine`` database — NOT an empty DB.

Run against k3d (full steps in tests/citation_engine/README.md):
    kubectl --context=k3d-srw -n srw port-forward svc/srw-pgvector 5433:5432 &
    export RUN_POSTGRES_TESTS=true
    export CITATION_DB_URL="postgresql://<citation_user>:<pw>@localhost:5433/citation_engine"
