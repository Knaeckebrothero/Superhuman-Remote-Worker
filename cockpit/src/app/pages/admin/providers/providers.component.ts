...
@Component({
                    ...
                    @for (model of endpoint.models; track model.id) {
                      <div class="model-row">
                        ...
                        <span class="col-model-action">
                          <button
                            class="revoke-btn"
                            (click)="deleteEndpointModel(endpoint.id, model.model_id)"
  ...
  styles: [`
    ...
    .model-row {
      display: grid;
      grid-template-columns: 2fr 1.5fr 1fr 0.8fr 70px;
      gap: 8px;
      align-items: center;
      padding: 6px 0;
      font-size: 12px;
...
export class AdminProvidersComponent implements OnInit {
  ...
  readonly defaultKinds: DefaultModelKind[] = ['builder', 'browser', 'citation'];
  ...
  deleteEndpointModel(endpointId: string, modelId: string): void {
    if (!confirm(this.transloco.translate('admin.providers.endpoints.confirmDeleteModel'))) return;
    this.admin.deleteSystemEndpointModel(endpointId, modelId).subscribe();
  }
